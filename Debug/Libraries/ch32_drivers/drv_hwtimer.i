# 1 "../Libraries/ch32_drivers/drv_hwtimer.c"
...
# 1 "<command-line>" 2
# 1 "../Libraries/ch32_drivers/drv_hwtimer.c"
# 11 "../Libraries/ch32_drivers/drv_hwtimer.c"
...
# 12 "../Libraries/ch32_drivers/drv_hwtimer.c" 2
...
# 13 "../Libraries/ch32_drivers/drv_hwtimer.c" 2
...
# 163 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD\\rt-thread\\include/rtdbg.h"
...
# 18 "../Libraries/ch32_drivers/drv_hwtimer.c" 2

# 1 "../Libraries/ch32_drivers/drv_hwtimer.h" 1
# 12 "../Libraries/ch32_drivers/drv_hwtimer.h"
...
# 20 "../Libraries/ch32_drivers/drv_hwtimer.h" 2
...
#define TIM_DEV_INFO_CONFIG { .maxfreq = 1000000, .minfreq = 3000, .maxcnt = 0xFFFF, .cntmode = HWTIMER_CNTMODE_UP, }
# 52 "../Libraries/ch32_drivers/drv_hwtimer.h"
#define TIM1_CONFIG { .handle.instance = TIM1, .handle.rcc = RCC_APB2Periph_TIM1, .irqn = TIM1_UP_IRQn, .name = "timer1", }
# 72 "../Libraries/ch32_drivers/drv_hwtimer.h"
#define TIM3_CONFIG { .handle.instance = TIM3, .handle.rcc = RCC_APB1Periph_TIM3, .irqn = TIM3_IRQn, .name = "timer3", }
# 82 "../Libraries/ch32_drivers/drv_hwtimer.h"
#define TIM4_CONFIG { .handle.instance = TIM4, .handle.rcc = RCC_APB1Periph_TIM4, .irqn = TIM4_IRQn, .name = "timer4", }
# 20 "../Libraries/ch32_drivers/drv_hwtimer.c" 2
...
# 21 "../Libraries/ch32_drivers/drv_hwtimer.c" 2
...
enum
{
    ...
    TIM4_INDEX,
# 56 "../Libraries/ch32_drivers/drv_hwtimer.c"
};

static struct ch32_hwtimer ch32_hwtimer_obj[] =
{

    { .handle.instance = ((TIM_TypeDef *)((((uint32_t)0x40000000) + 0x10000) + 0x2C00)), .handle.rcc = ((uint32_t)0x00000800), .irqn = TIM1_UP_IRQn, .name = "timer1", },
    ...
    { .handle.instance = ((TIM_TypeDef *)((((uint32_t)0x40000000)) + 0x0400)), .handle.rcc = ((uint32_t)0x00000002), .irqn = TIM3_IRQn, .name = "timer3", },



    { .handle.instance = ((TIM_TypeDef *)((((uint32_t)0x40000000)) + 0x0800)), .handle.rcc = ((uint32_t)0x00000004), .irqn = TIM4_IRQn, .name = "timer4", },
# 99 "../Libraries/ch32_drivers/drv_hwtimer.c"
};


void ch32_get_pclk_doubler(rt_uint32_t *pclk1_doubler, rt_uint32_t *pclk2_doubler)
{
    if (!(pclk1_doubler != 0)) { rt_assert_handler("pclk1_doubler != RT_NULL", __FUNCTION__, 104); };
    if (!(pclk2_doubler != 0)) { rt_assert_handler("pclk2_doubler != RT_NULL", __FUNCTION__, 105); };

    *pclk1_doubler = 1;
    *pclk2_doubler = 1;

    if((((RCC_TypeDef *)((((uint32_t)0x40000000) + 0x20000) + 0x1000))->CFGR0 & ((uint32_t)0x00000700)) == ((uint32_t)0x00000000))
    {
        *pclk1_doubler = 1;
    }
    else
    ...
    if((((RCC_TypeDef *)((((uint32_t)0x40000000) + 0x20000) + 0x1000))->CFGR0 & ((uint32_t)0x00003800)) == ((uint32_t)0x00000000))
    {
        *pclk2_doubler = 1;
    }
    else
...
{
    if (!(timer != 0)) { rt_assert_handler("timer != RT_NULL", __FUNCTION__, 131); };
    TIM_HandleTypeDef *tim = 0;
    RCC_ClocksTypeDef RCC_ClockStruct;
    NVIC_InitTypeDef NVIC_InitStruct;
    struct ch32_hwtimer *tim_device = 0;
    ...
    if(state)
    {
        ...
        if(tim->instance == ((TIM_TypeDef *)((((uint32_t)0x40000000) + 0x10000) + 0x2C00)))
        ...
        tim->init.TIM_ClockDivision = ((uint16_t)0x0000);
        tim->init.TIM_Period = 10000 - 1;
        tim->init.TIM_RepetitionCounter = 0;

        if(timer->info->cntmode == 0x01)
        {
            tim->init.TIM_CounterMode = ((uint16_t)0x0000);
        }
        else
        {
            tim->init.TIM_CounterMode = ((uint16_t)0x0010);
        }
        ...
        TIM_ClearITPendingBit(tim->instance, ((uint16_t)0x0001));
        TIM_ITConfig(tim->instance, ((uint16_t)0x0001), ENABLE);
    }
}

static rt_err_t ch32_hwtimer_start(struct rt_hwtimer_device *timer, rt_uint32_t cnt, rt_hwtimer_mode_t mode)
{
    if (!(timer != 0)) { rt_assert_handler("timer != RT_NULL", __FUNCTION__, 200); };
    TIM_HandleTypeDef *tim = 0;
    tim = (TIM_HandleTypeDef *)timer->parent.user_data;
    ...
    if (mode == HWTIMER_MODE_ONESHOT)
    {
        ...
        tim->instance->CTLR1 &= (rt_uint16_t) ~((rt_uint16_t)((uint16_t)0x0008));
        tim->instance->CTLR1 |= ((uint16_t)0x0008);
    }
    else
    {
        tim->instance->CTLR1 &= (rt_uint16_t) ~((rt_uint16_t)((uint16_t)0x0008));
        tim->instance->CTLR1 |= ((uint16_t)0x0000);
    }
...
{
    if (!(timer != 0)) { rt_assert_handler("timer != RT_NULL", __FUNCTION__, 230); };
    TIM_HandleTypeDef *tim = 0;
...
{
    if (!(timer != 0)) { rt_assert_handler("timer != RT_NULL", __FUNCTION__, 244); };
    TIM_HandleTypeDef *tim = 0;
    tim = (TIM_HandleTypeDef *)timer->parent.user_data;
...
{
    if (!(timer != 0)) { rt_assert_handler("timer != RT_NULL", __FUNCTION__, 253); };
    if (!(args != 0)) { rt_assert_handler("args != RT_NULL", __FUNCTION__, 254); };
    ...
    case HWTIMER_CTRL_FREQ_SET:
    {
        ...
        TIM_PrescalerConfig(tim->instance, val - 1, ((uint16_t)0x0001));
...
static const struct rt_hwtimer_info ch32_hwtimer_info = { .maxfreq = 1000000, .minfreq = 3000, .maxcnt = 0xFFFF, .cntmode = 0x01, };
...
{
    if (!(device != 0)) { rt_assert_handler("device != RT_NULL", __FUNCTION__, 350); };
    struct ch32_hwtimer *hwtimer = 0;
    hwtimer = ((struct ch32_hwtimer *)((char *)(device) - (unsigned long)(&((struct ch32_hwtimer *)0)->device)));

    if(TIM_GetITStatus(hwtimer->handle.instance, ((uint16_t)0x0001)) != RESET)
    {
        rt_device_hwtimer_isr(device);
        TIM_ClearITPendingBit(hwtimer->handle.instance, ((uint16_t)0x0001));
    }
}
...
# 386 "../Libraries/ch32_drivers/drv_hwtimer.c"
...
# 481 "../Libraries/ch32_drivers/drv_hwtimer.c"
static int rt_hw_timer_init(void)
{
    ...
    for (i = 0; i < sizeof(ch32_hwtimer_obj) / sizeof(ch32_hwtimer_obj[0]); i++)
    {
        ...
        if (!(result == 0)) { rt_assert_handler("result == RT_EOK", __FUNCTION__, 492); };
    }

    return result;
}
__attribute__((used)) const init_fn_t __rt_init_rt_hw_timer_init __attribute__((section(".rti_fn." "1"))) = rt_hw_timer_init;
