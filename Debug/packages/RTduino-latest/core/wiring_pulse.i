# 1 "../packages/RTduino-latest/core/wiring_pulse.c"
...
# 1 "<command-line>" 2
# 1 "../packages/RTduino-latest/core/wiring_pulse.c"
# 14 "../packages/RTduino-latest/core/wiring_pulse.c"
...
# 15 "../packages/RTduino-latest/core/wiring_pulse.c" 2
...
# 16 "../packages/RTduino-latest/core/wiring_pulse.c" 2
...
# 20 "../packages/RTduino-latest/core/wiring_pulse.c" 2

static struct rt_semaphore pulsein_sem;
static rt_bool_t pulsein_sem_init_flag = 0;
static struct
{
...
{
    if(pulse_record.first_pulse_coming == 0
        && pulse_record.second_pulse_coming == 0
        && rt_pin_read(pulse_record.rt_isr_pin) == pulse_record.state)
    {
        pulse_record.first_pulse_timestamp_us = micros();
        pulse_record.first_pulse_coming = 1;
    }
    else if(pulse_record.first_pulse_coming == 1
        && pulse_record.second_pulse_coming == 0
        && rt_pin_read(pulse_record.rt_isr_pin) != pulse_record.state)
    {
        pulse_record.second_pulse_timestamp_us = micros();
        pulse_record.second_pulse_coming = 1;
        rt_sem_release(&pulsein_sem);
    }
    else
    {
...
# 64 "../packages/RTduino-latest/core/wiring_pulse.c"
unsigned long pulseIn(uint8_t pin, uint8_t state, unsigned long timeout)
{
    rt_base_t level;
    unsigned long delta;
    ...
    if (!(state == 0x1 || state == 0x0)) { rt_assert_handler("state == HIGH || state == LOW", __FUNCTION__, 71); };

    ;

    if(pulsein_sem_init_flag == 0)
    {
        rt_sem_init(&pulsein_sem, "pulsein", 0, 0x01);
        pulsein_sem_init_flag = 1;
    }
    ...
    pulsein_sem.value = 0;
    pulse_record.first_pulse_coming = 0;
    ...
    rt_pin_attach_irq(rt_pin, 0x02, pulsein_pin_interrupt_cb, 0);
    rt_pin_irq_enable(rt_pin, 0x01);
    rt_err = rt_sem_take(&pulsein_sem, rt_tick_from_millisecond(timeout/1000));
    rt_pin_irq_enable(rt_pin, 0x00);
    ...
    {
        do { rt_kprintf("[" "E" "/" "RTduino.pulse" "] "); rt_kprintf("semaphore error: %d", rt_err); rt_kprintf("\n"); } while (0);
        delta = 0;
    }
    return delta;
}
# 121 "../packages/RTduino-latest/core/wiring_pulse.c"
unsigned long pulseInLong(uint8_t pin, uint8_t state, unsigned long timeout)
{
