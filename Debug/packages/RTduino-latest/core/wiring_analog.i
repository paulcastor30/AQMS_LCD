# 1 "../packages/RTduino-latest/core/wiring_analog.c"
...
# 1 "<command-line>" 2
# 1 "../packages/RTduino-latest/core/wiring_analog.c"
# 14 "../packages/RTduino-latest/core/wiring_analog.c"
...
# 15 "../packages/RTduino-latest/core/wiring_analog.c" 2
...
# 16 "../packages/RTduino-latest/core/wiring_analog.c" 2
...
# 21 "../packages/RTduino-latest/core/wiring_analog.c" 2

static uint8_t _analog_read_resolution = 10;
static uint8_t _analog_write_resolution = 8;
static uint32_t _analog_pwm_hz = 500;

#define PWM_PERIOD_NS (1000*1000*1000 /_analog_pwm_hz)

void analogReference(uint8_t mode)
{
    do { rt_kprintf("[" "E" "/" "RTduino.analog" "] "); rt_kprintf("analogReference() has not been implemented yet!"); rt_kprintf("\n"); } while (0);
}
...
void analogWrite(uint8_t pin, int val)
{
    ...
    if(pwm_dev != 0 && pwm_dev->parent.type == RT_Device_Class_PWM)
    {
        rt_pwm_val = map(val, 0, (1UL << (_analog_write_resolution))-1, 0, (1000*1000*1000 /_analog_pwm_hz));
        rt_pwm_disable(pwm_dev, pin_map_table[pin].channel);
        rt_pwm_set(pwm_dev, pin_map_table[pin].channel, (1000*1000*1000 /_analog_pwm_hz), rt_pwm_val);
        rt_pwm_enable(pwm_dev, pin_map_table[pin].channel);
        return;
    }
# 91 "../packages/RTduino-latest/core/wiring_analog.c"
    pinMode(pin, 0x1);
    if (val < (1UL << (_analog_write_resolution))/2)
    {
...
int analogRead(uint8_t pin)
{
    ...
    if(adc_dev != 0 && adc_dev->parent.type == RT_Device_Class_ADC)
    {
        ...
        if(rt_device_control((rt_device_t)adc_dev, RT_ADC_CMD_GET_RESOLUTION, &resolution) == 0)
        {
            return map(rt_adc_val, 0, (1UL << (resolution))-1, 0, (1UL << (_analog_read_resolution))-1);
        }
        else
        {
            do { rt_kprintf("[" "W" "/" "RTduino.analog" "] "); rt_kprintf("This board doesn't support to adjust ADC resolution. It will return real ADC value!"); rt_kprintf("\n"); } while (0);
            return rt_adc_val;
        }
    }
    else
    ...
    {
        do { rt_kprintf("[" "E" "/" "RTduino.analog" "] "); rt_kprintf("This pin %d doesn't support ADC", pin); rt_kprintf("\n"); } while (0);
        return 0;
    }
}
