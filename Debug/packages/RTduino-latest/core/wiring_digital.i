# 1 "../packages/RTduino-latest/core/wiring_digital.c"
...
# 1 "<command-line>" 2
# 1 "../packages/RTduino-latest/core/wiring_digital.c"
# 14 "../packages/RTduino-latest/core/wiring_digital.c"
...
# 15 "../packages/RTduino-latest/core/wiring_digital.c" 2
...
# 18 "../packages/RTduino-latest/core/wiring_digital.c" 2
...
# 19 "../packages/RTduino-latest/core/wiring_digital.c" 2
...
# 23 "../packages/RTduino-latest/core/wiring_digital.c" 2

void pinMode(uint8_t pin, uint8_t mode)
{
    rt_base_t rt_mode;

    if (!(mode == 0x0 || mode == 0x1 || mode == 0x2 || mode == 0x3 || mode == 0x4)) { rt_assert_handler("mode == INPUT || mode == OUTPUT || mode == INPUT_PULLUP || mode == INPUT_PULLDOWN || mode == OUTPUT_OPEN_DRAIN", __FUNCTION__,

                                       30
# 28 "../packages/RTduino-latest/core/wiring_digital.c"
    ); }
    ...
    if (pin_map_table[pin].device_name != 0)
    {
        do { rt_kprintf("[" "W" "/" "RTduino.digital" "] "); rt_kprintf("[pinMode] Pin %d was a %s device IO, but now it becomes a general-purpose IO.", pin, pin_map_table[pin].device_name); rt_kprintf("\n"); } while (0);
    }
...
void digitalWrite(uint8_t pin, uint8_t val)
{
    ...
    if (!(val == 0x1 || val == 0x0)) { rt_assert_handler("val == HIGH || val == LOW", __FUNCTION__, 68); };
...
{
    ;
    return (rt_pin_read(pin_map_table[pin].rt_pin) == 0x01)? 0x1 : 0x0;
}
