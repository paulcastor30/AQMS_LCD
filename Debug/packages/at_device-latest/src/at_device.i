# 1 "../packages/at_device-latest/src/at_device.c"
...
# 1 "<command-line>" 2
# 1 "../packages/at_device-latest/src/at_device.c"
# 11 "../packages/at_device-latest/src/at_device.c"
...
# 12 "c:\\rt-threadstudio\\repo\\extract\\toolchain_support_packages\\wch\\risc-v-gcc-wch\\8.2.0\\riscv-none-embed\\include\\stdlib.h" 2 3
...
# 19 "c:\\rt-threadstudio\\repo\\extract\\toolchain_support_packages\\wch\\risc-v-gcc-wch\\8.2.0\\riscv-none-embed\\include\\stdlib.h" 2 3
...
# 20 "c:\\rt-threadstudio\\repo\\extract\\toolchain_support_packages\\wch\\risc-v-gcc-wch\\8.2.0\\riscv-none-embed\\include\\stdlib.h" 2 3
...
# 12 "../packages/at_device-latest/src/at_device.c" 2
...
# 13 "../packages/at_device-latest/src/at_device.c" 2
...
# 15 "../packages/at_device-latest/src/at_device.c" 2
...
# 19 "../packages/at_device-latest/src/at_device.c" 2


static rt_slist_t at_device_list = { 0 };

static rt_slist_t at_device_class_list = { 0 };
...
struct at_device *at_device_get_first_initialized(void)
{
    ...
    for (node = (&at_device_list)->next; node != 0; node = node->next)
    {
        device = ((struct at_device *)((char *)(node) - (unsigned long)(&((struct at_device *)0)->list)));
        if (device && device->is_init == 1)
        {
...
# 61 "../packages/at_device-latest/src/at_device.c"
struct at_device *at_device_get_by_name(int type, const char *name)
{
    rt_base_t level;
    rt_slist_t *node = 0;
    ...
    if (!(name)) { rt_assert_handler("name", __FUNCTION__, 67); };
...
# 105 "../packages/at_device-latest/src/at_device.c"
struct at_device *at_device_get_by_ipaddr(ip_addr_t *ip_addr)
{
    ...
    {
        device = ((struct at_device *)((char *)(node) - (unsigned long)(&((struct at_device *)0)->list)));
        if (device && ((ip_addr)->addr == (&(device->netdev->ip_addr))->addr))
        {
           rt_hw_interrupt_enable(level);
           return device;
        }
...
# 141 "../packages/at_device-latest/src/at_device.c"
int at_device_control(struct at_device *device, int cmd, void *arg)
{
    ...
    {
        do { rt_kprintf("[" "W" "/" "at.dev" "] "); rt_kprintf("AT device(%s) not support control operations.", device->name); rt_kprintf("\n"); } while (0);
        return 0;
    }
}
# 162 "../packages/at_device-latest/src/at_device.c"
int at_device_class_register(struct at_device_class *class, uint16_t class_id)
{
    rt_base_t level;

    if (!(class)) { rt_assert_handler("class", __FUNCTION__, 166); };
...
static struct at_device_class *at_device_class_get(uint16_t class_id)
{
    ...
    for (node = (&at_device_class_list)->next; node != 0; node = node->next)
    {
        class = ((struct at_device_class *)((char *)(node) - (unsigned long)(&((struct at_device_class *)0)->list)));
...
# 221 "../packages/at_device-latest/src/at_device.c"
int at_device_register(struct at_device *device, const char *device_name,
                        const char *at_client_name, uint16_t class_id, void *user_data)
{
    rt_base_t level;
    ...
    if (!(device)) { rt_assert_handler("device", __FUNCTION__, 230); };
    if (!(device_name)) { rt_assert_handler("device_name", __FUNCTION__, 231); };
    if (!(at_client_name)) { rt_assert_handler("at_client_name", __FUNCTION__, 232); };

    class = at_device_class_get(class_id);
    if (class == 0)
    {
        do { rt_kprintf("[" "E" "/" "at.dev" "] "); rt_kprintf("get AT device class(%d) failed.", class_id); rt_kprintf("\n"); } while (0);
        result = -1;
        goto __exit;
    }
    ...
    if (device->sockets == 0)
    {
        do { rt_kprintf("[" "E" "/" "at.dev" "] "); rt_kprintf("no memory for AT Socket number(%d) create.", class->socket_num); rt_kprintf("\n"); } while (0);
        result = -5;
        goto __exit;
    }


    rt_snprintf(name, 8, "at_se%d", device_counts++);
    device->socket_event = rt_event_create(name, 0x00);
    if (device->socket_event == 0)
    {
        do { rt_kprintf("[" "E" "/" "at.dev" "] "); rt_kprintf("no memory for AT device(%s) socket event create.", device_name); rt_kprintf("\n"); } while (0);
        result = -5;
        goto __exit;
    }
