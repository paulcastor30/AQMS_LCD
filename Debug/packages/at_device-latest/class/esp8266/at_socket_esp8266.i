# 1 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c"
...
# 1 "<command-line>" 2
# 1 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c"
# 13 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c"
...
# 14 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c" 2
...
# 15 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c" 2
...
# 17 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c" 2
...
# 20 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c" 2
...
static at_evt_cb_t at_evt_cb_set[] = {
        [AT_SOCKET_EVT_RECV] = 
# 37 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c" 3 4
                              ((void *)0)
# 37 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c"
                                  ,
        [AT_SOCKET_EVT_CLOSED] = 
# 38 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c" 3 4
                                ((void *)0)
# 38 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c"
                                    ,
...
# 77 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c"
...
static int esp8266_socket_event_recv(struct at_device *device, uint32_t event, uint32_t timeout, rt_uint8_t option)
{
    ...
    result = rt_event_recv(device->socket_event, event, option | 0x04, timeout, &recved);
    if (result != 0)
    {
        return -2;
    }
...
# 106 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c"
static int esp8266_socket_close(struct at_socket *socket)
{
    ...
    if (resp == 0)
    {
        do { rt_kprintf("[" "E" "/" "at.skt.esp" "] "); rt_kprintf("no memory for resp create."); rt_kprintf("\n"); } while (0);
        return -5;
    }
...
# 144 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c"
static int esp8266_socket_connect(struct at_socket *socket, char *ip, int32_t port, enum at_socket_type type, rt_bool_t is_client)
{
    ...
    if (!(ip)) { rt_assert_handler("ip", __FUNCTION__, 152); };
    if (!(port >= 0)) { rt_assert_handler("port >= 0", __FUNCTION__, 153); };
...
__retry:
    if (is_client)
    {
        ...
        default:
            do { rt_kprintf("[" "E" "/" "at.skt.esp" "] "); rt_kprintf("not supported connect type %d.", type); rt_kprintf("\n"); } while (0);
            result = -1;
            goto __exit;
        }
    }

    if (result != 0 && retryed == 0)
    {
...
# 288 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c"
static int esp8266_socket_send(struct at_socket *socket, const char *buff, size_t bfsz, enum at_socket_type type)
{
    ...
    if (!(buff)) { rt_assert_handler("buff", __FUNCTION__, 299); };
    if (!(bfsz > 0)) { rt_assert_handler("bfsz > 0", __FUNCTION__, 300); };

    resp = at_create_resp(128, 2, 5 * 1000);
    if (resp == 0)
    {
        do { rt_kprintf("[" "E" "/" "at.skt.esp" "] "); rt_kprintf("no memory for resp create."); rt_kprintf("\n"); } while (0);
        return -5;
    ...
    while (sent_size < bfsz)
    {
        ...
        if (esp8266_socket_event_recv(device, (((device_socket + 1) << 16) | (0)),
                                      10 * 1000, 0x02) < 0)
        {
            do { rt_kprintf("[" "E" "/" "at.skt.esp" "] "); rt_kprintf("%s device socket(%d) wait connect result timeout.", device->name, device_socket); rt_kprintf("\n"); } while (0);
            result = -2;
            goto __exit;
        }

        event_result = esp8266_socket_event_recv(device, (1L << 1) | (1L << 5),
                                                 5 * 1000, 0x02);
        if (event_result < 0)
        {
            do { rt_kprintf("[" "E" "/" "at.skt.esp" "] "); rt_kprintf("%s device socket(%d) wait connect OK|FAIL timeout.", device->name, device_socket); rt_kprintf("\n"); } while (0);
            result = -2;
            goto __exit;
        }

        if (event_result & (1L << 5))
        {
            do { rt_kprintf("[" "E" "/" "at.skt.esp" "] "); rt_kprintf("%s device socket(%d) send failed.", device->name, device_socket); rt_kprintf("\n"); } while (0);
            result = -1;
            goto __exit;
        }
...
# 401 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c"
...
#define RESOLVE_RETRY 5
    ...
    if (!(name)) { rt_assert_handler("name", __FUNCTION__, 410); };
    if (!(ip)) { rt_assert_handler("ip", __FUNCTION__, 411); };

    device = at_device_get_first_initialized();
    if (device == 0)
    {
        do { rt_kprintf("[" "E" "/" "at.skt.esp" "] "); rt_kprintf("get first init device failed."); rt_kprintf("\n"); } while (0);
        return -1;
    }
...
static void urc_send_func(struct at_client *client, const char *data, rt_size_t size)
{
    int device_socket = 0;
    struct at_device *device = 0;
    struct at_device_esp8266 *esp8266 = 0;
    char *client_name = client->device->parent.name;

    if (!(data && size)) { rt_assert_handler("data && size", __FUNCTION__, 503); };

    device = at_device_get_by_name(0x03, client_name);
    if (device == 0)
    {
        do { rt_kprintf("[" "E" "/" "at.skt.esp" "] "); rt_kprintf("get device(%s) failed.", client_name); rt_kprintf("\n"); } while (0);
        return;
    }
    ...
    {
        esp8266_socket_event_send(device, (((device_socket + 1) << 16) | ((1L << 1))));
    }
    else if (rt_strstr(data, "SEND FAIL"))
    {
        esp8266_socket_event_send(device, (((device_socket + 1) << 16) | ((1L << 5))));
    }
}
...
static void urc_send_bfsz_func(struct at_client *client, const char *data, rt_size_t size)
{
    ...
    if (!(data && size)) { rt_assert_handler("data && size", __FUNCTION__, 528); };
...
{
    int index = 0;
    struct at_socket *socket = 0;
    struct at_device *device = 0;
    char *client_name = client->device->parent.name;

    if (!(data && size)) { rt_assert_handler("data && size", __FUNCTION__, 540); };
    ...
    {
        at_evt_cb_set[AT_SOCKET_EVT_CLOSED](socket, AT_SOCKET_EVT_CLOSED, 0, 0);
    }
}
# 592 "../packages/at_device-latest/class/esp8266/at_socket_esp8266.c"
static void urc_recv_func(struct at_client *client, const char *data, rt_size_t size)
{
    int device_socket = 0;
    rt_int32_t timeout = 0;
    rt_size_t bfsz = 0, temp_size = 0;
    char *recv_buf = 0, temp[8] = {0};
    struct at_socket *socket = 0;
    struct at_device *device = 0;
    char *client_name = client->device->parent.name;

    if (!(data && size)) { rt_assert_handler("data && size", __FUNCTION__, 602); };
    ...
    if (recv_buf == 0)
    {
        do { rt_kprintf("[" "E" "/" "at.skt.esp" "] "); rt_kprintf("no memory receive buffer(%d).", bfsz); rt_kprintf("\n"); } while (0);
    ...
    {
        do { rt_kprintf("[" "E" "/" "at.skt.esp" "] "); rt_kprintf("%s device receive size(%d) data failed.", device->name, bfsz); rt_kprintf("\n"); } while (0);
        rt_free(recv_buf);
        return;
    }
...
int esp8266_socket_init(struct at_device *device)
{
    if (!(device)) { rt_assert_handler("device", __FUNCTION__, 664); };
...
int esp8266_socket_class_register(struct at_device_class *class)
{
    if (!(class)) { rt_assert_handler("class", __FUNCTION__, 674); };
