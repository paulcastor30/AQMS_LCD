# 1 "../rt-thread/components/drivers/hwtimer/hwtimer.c"
...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/drivers/hwtimer/hwtimer.c"
# 11 "../rt-thread/components/drivers/hwtimer/hwtimer.c"
...
# 12 "../rt-thread/components/drivers/hwtimer/hwtimer.c" 2
...
# 17 "../rt-thread/components/drivers/hwtimer/hwtimer.c" 2
...
static rt_err_t rt_hwtimer_control(struct rt_device *dev, int cmd, void *args)
{
    ...
    case HWTIMER_CTRL_FREQ_SET:
    {
        ...
        {
            do { rt_kprintf("[" "W" "/" "hwtimer" "] "); rt_kprintf("frequency setting out of range! It will maintain at %d Hz", timer->freq); rt_kprintf("\n"); } while (0);
            result = -10;
            break;
        }
...
void rt_device_hwtimer_isr(rt_hwtimer_t *timer)
{
    ...
    if (!(timer != 0)) { rt_assert_handler("timer != RT_NULL", __FUNCTION__, 313); };
...
# 361 "../rt-thread/components/drivers/hwtimer/hwtimer.c"
rt_err_t rt_device_hwtimer_register(rt_hwtimer_t *timer, const char *name, void *user_data)
{
    struct rt_device *device;

    if (!(timer != 0)) { rt_assert_handler("timer != RT_NULL", __FUNCTION__, 365); };
    if (!(timer->ops != 0)) { rt_assert_handler("timer->ops != RT_NULL", __FUNCTION__, 366); };
    if (!(timer->info != 0)) { rt_assert_handler("timer->info != RT_NULL", __FUNCTION__, 367); };
