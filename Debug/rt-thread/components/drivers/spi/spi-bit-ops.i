# 1 "../rt-thread/components/drivers/spi/spi-bit-ops.c"
...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/drivers/spi/spi-bit-ops.c"
# 11 "../rt-thread/components/drivers/spi/spi-bit-ops.c"
...
# 16 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD\\rt-thread\\components\\drivers\\spi/spi-bit-ops.h" 2
...
rt_err_t rt_spi_bit_add_bus(struct rt_spi_bit_obj *obj,
                            const char *bus_name,
                            struct rt_spi_bit_ops *ops);
# 12 "../rt-thread/components/drivers/spi/spi-bit-ops.c" 2
...
# 154 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD\\rt-thread\\include/rtdbg.h"
...
# 21 "../rt-thread/components/drivers/spi/spi-bit-ops.c" 2
...
static __inline void spi_delay(struct rt_spi_bit_ops *ops)
{
    ops->udelay((ops->delay_us + 1) >> 1);
}

static __inline void spi_delay2(struct rt_spi_bit_ops *ops)
{
...
static __inline rt_ssize_t spi_xfer_4line_data8(struct rt_spi_bit_ops *ops,
                                         struct rt_spi_configuration *config,
                                         const void *send_buf,
                                         void *recv_buf,
                                         rt_size_t length)
{
...

    {
        ...
        while (size--)
        {
            ...
            for (i = 0; i < 8; i++)
            {
                if (config->mode & (1<<2)) { bit = tx_data & (0x1 << (7 - i)); }
                else { bit = tx_data & (0x1 << i); }

                if (bit) ops->set_mosi(ops->data, 1);
                else ops->set_mosi(ops->data, 0);

                spi_delay2(ops);

                ops->tog_sclk(ops->data);

                if (config->mode & (1<<2)) { rx_data <<= 1; bit = 0x01; }
                else { rx_data >>= 1; bit = 0x80; }

                if (ops->get_miso(ops->data)) { rx_data |= bit; }
                else { rx_data &= ~bit; }

                spi_delay2(ops);

                if (!(config->mode & (1<<0)) || (size != 0) || (i < 7))
                {
                    ops->tog_sclk(ops->data);
                }
            }
...
static __inline rt_ssize_t spi_xfer_4line_data16(struct rt_spi_bit_ops *ops,
                                          struct rt_spi_configuration *config,
                                          const void *send_buf,
                                          void *recv_buf,
                                          rt_size_t length)
{
...

    {
        ...
        while (size--)
        {
            ...
            for (i = 0; i < 16; i++)
            {
                if (config->mode & (1<<2)) { bit = tx_data & (0x1 << (15 - i)); }
                else { bit = tx_data & (0x1 << i); }
                ...
                if (config->mode & (1<<2)) { rx_data <<= 1; bit = 0x0001; }
                ...
                if (!(config->mode & (1<<0)) || (size != 0) || (i < 15))
                {
                    ops->tog_sclk(ops->data);
                }
            }
...
static __inline rt_ssize_t spi_xfer_3line_data8(struct rt_spi_bit_ops *ops,
                                         struct rt_spi_configuration *config,
                                         const void *send_buf,
                                         void *recv_buf,
                                         rt_size_t length)
{
...

    {
        ...
        {
            ops->dir_mosi(ops->data, 0);
            send_flg = 1;
        }
        else
        {
            ops->dir_mosi(ops->data, 1);
        }
        ...
        while (size--)
        {
            ...
            else
            {
                for (i = 0; i < 8; i++)
                {
                    ...
                    if (ops->get_mosi(ops->data)) { rx_data |= bit; }
                    else { rx_data &= ~bit; }
...
static __inline rt_ssize_t spi_xfer_3line_data16(struct rt_spi_bit_ops *ops,
                                          struct rt_spi_configuration *config,
                                          const void *send_buf,
                                          void *recv_buf,
                                          rt_size_t length)
...
{
    struct rt_spi_bit_obj *obj = ((struct rt_spi_bit_obj *)((char *)(device->bus) - (unsigned long)(&((struct rt_spi_bit_obj *)0)->bus)));
    struct rt_spi_bit_ops *ops = obj->ops;

    ;
    ;

    if (configuration->mode & (1<<3))
    {
        return -8;
    }

    if (configuration->mode & (1<<1))
    {
        ops->set_sclk(ops->data, 1);
    }
    else
    {
        ops->set_sclk(ops->data, 0);
    }
...
rt_ssize_t spi_bit_xfer(struct rt_spi_device *device, struct rt_spi_message *message)
{
    ...
    ;
    ;
# 441 "../rt-thread/components/drivers/spi/spi-bit-ops.c"
    if (message->cs_take && (cs_pin != (-1)))
    {
        ;
        rt_pin_write(cs_pin, 0x00);
        spi_delay(ops);
    ...
    if (message->cs_release && (cs_pin != (-1)))
    {
        spi_delay(ops);
        rt_pin_write(cs_pin, 0x01);
        ;
    }
                            ...
                            struct rt_spi_bit_ops *ops)
{
    ...
    obj->config.mode = (0<<3) | (1<<2) | (0 | 0);


    if (obj->config.mode & (1<<1)) ops->set_sclk(ops->data, 1);
    else ops->set_sclk(ops->data, 0);
