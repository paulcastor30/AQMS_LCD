# 1 "../rt-thread/components/drivers/core/device.c"
...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/drivers/core/device.c"
# 18 "../rt-thread/components/drivers/core/device.c"
...
# 19 "../rt-thread/components/drivers/core/device.c" 2
...
# 27 "../rt-thread/components/drivers/core/device.c" 2
# 42 "../rt-thread/components/drivers/core/device.c"
...
# 61 "../rt-thread/components/drivers/core/device.c"
rt_err_t rt_device_register(rt_device_t dev,
                            const char *name,
                            rt_uint16_t flags)
{
...
;
# 92 "../rt-thread/components/drivers/core/device.c"
rt_err_t rt_device_unregister(rt_device_t dev)
{

    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 95); };
    if (!(rt_object_get_type(&dev->parent) == RT_Object_Class_Device)) { rt_assert_handler("rt_object_get_type(&dev->parent) == RT_Object_Class_Device", __FUNCTION__, 96); };
    if (!(rt_object_is_systemobject(&dev->parent))) { rt_assert_handler("rt_object_is_systemobject(&dev->parent)", __FUNCTION__, 97); };
...
;
# 112 "../rt-thread/components/drivers/core/device.c"
rt_device_t rt_device_find(const char *name)
{
    return (rt_device_t)rt_object_find(name, RT_Object_Class_Device);
}
;
# 128 "../rt-thread/components/drivers/core/device.c"
rt_device_t rt_device_create(int type, int attach_size)
{
    int size;
    rt_device_t device;
...
void rt_device_destroy(rt_device_t dev)
{
    ...
    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 157); };
    if (!(rt_object_get_type(&dev->parent) == RT_Object_Class_Device)) { rt_assert_handler("rt_object_get_type(&dev->parent) == RT_Object_Class_Device", __FUNCTION__, 158); };
    if (!(rt_object_is_systemobject(&dev->parent) == 0)) { rt_assert_handler("rt_object_is_systemobject(&dev->parent) == RT_FALSE", __FUNCTION__, 159); };
...
;
# 176 "../rt-thread/components/drivers/core/device.c"
rt_err_t rt_device_init(rt_device_t dev)
{
    rt_err_t result = 0;

    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 180); };
    ...
    if ((dev->init) != 0)
    {
        if (!(dev->flag & 0x010))
        {
            ...
            if (result != 0)
            {
                do { rt_kprintf("[" "E" "/" "kernel.device" "] "); rt_kprintf("To initialize device:%s failed. The error code is %d", dev->parent.name, result); rt_kprintf("\n"); } while (0)
                                               ;
            }
            else
            {
...
# 212 "../rt-thread/components/drivers/core/device.c"
rt_err_t rt_device_open(rt_device_t dev, rt_uint16_t oflag)
{
    rt_err_t result = 0;


    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 217); };
    if (!(rt_object_get_type(&dev->parent) == RT_Object_Class_Device)) { rt_assert_handler("rt_object_get_type(&dev->parent) == RT_Object_Class_Device", __FUNCTION__, 218); };
    ...
    if (!(dev->open_flag & 0x008) ||
         ((dev->open_flag & 0xf0f) != (oflag & 0xf0f)))
    {
        if ((dev->open) != 0)
        {
    ...
    if (result == 0 || result == -6)
    {
        ...
        if (!(dev->ref_count != 0)) { rt_assert_handler("dev->ref_count != 0", __FUNCTION__, 268); };
    }

    return result;
}
;
# 282 "../rt-thread/components/drivers/core/device.c"
rt_err_t rt_device_close(rt_device_t dev)
{
    rt_err_t result = 0;


    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 287); };
    if (!(rt_object_get_type(&dev->parent) == RT_Object_Class_Device)) { rt_assert_handler("rt_object_get_type(&dev->parent) == RT_Object_Class_Device", __FUNCTION__, 288); };
...
;
# 327 "../rt-thread/components/drivers/core/device.c"
rt_ssize_t rt_device_read(rt_device_t dev,
                         rt_off_t pos,
                         void *buffer,
                         rt_size_t size)
{
    ...
    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 333); };
    if (!(rt_object_get_type(&dev->parent) == RT_Object_Class_Device)) { rt_assert_handler("rt_object_get_type(&dev->parent) == RT_Object_Class_Device", __FUNCTION__, 334); };
...
;
# 370 "../rt-thread/components/drivers/core/device.c"
rt_ssize_t rt_device_write(rt_device_t dev,
                          rt_off_t pos,
                          const void *buffer,
                          rt_size_t size)
{
    ...
    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 376); };
    if (!(rt_object_get_type(&dev->parent) == RT_Object_Class_Device)) { rt_assert_handler("rt_object_get_type(&dev->parent) == RT_Object_Class_Device", __FUNCTION__, 377); };
...
;
# 409 "../rt-thread/components/drivers/core/device.c"
rt_err_t rt_device_control(rt_device_t dev, int cmd, void *arg)
{

    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 412); };
    if (!(rt_object_get_type(&dev->parent) == RT_Object_Class_Device)) { rt_assert_handler("rt_object_get_type(&dev->parent) == RT_Object_Class_Device", __FUNCTION__, 413); };
...
;
# 435 "../rt-thread/components/drivers/core/device.c"
rt_err_t rt_device_set_rx_indicate(rt_device_t dev,
                                   rt_err_t (*rx_ind)(rt_device_t dev,
                                   rt_size_t size))
{

    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 440); };
    if (!(rt_object_get_type(&dev->parent) == RT_Object_Class_Device)) { rt_assert_handler("rt_object_get_type(&dev->parent) == RT_Object_Class_Device", __FUNCTION__, 441); };
...
;
# 459 "../rt-thread/components/drivers/core/device.c"
rt_err_t rt_device_set_tx_complete(rt_device_t dev,
                                   rt_err_t (*tx_done)(rt_device_t dev,
                                   void *buffer))
{

    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 464); };
    if (!(rt_object_get_type(&dev->parent) == RT_Object_Class_Device)) { rt_assert_handler("rt_object_get_type(&dev->parent) == RT_Object_Class_Device", __FUNCTION__, 465); };
