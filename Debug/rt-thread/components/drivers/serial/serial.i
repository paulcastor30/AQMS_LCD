...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/drivers/serial/serial.c"
# 31 "../rt-thread/components/drivers/serial/serial.c"
...
#define rt_hw_dmb() 
#define rt_hw_dsb() 
# 32 "../rt-thread/components/drivers/serial/serial.c" 2
...
# 33 "../rt-thread/components/drivers/serial/serial.c" 2
...
# 34 "../rt-thread/components/drivers/serial/serial.c" 2
...
# 38 "../rt-thread/components/drivers/serial/serial.c" 2
...
static __inline int _serial_poll_rx(struct rt_serial_device *serial, rt_uint8_t *data, int length)
{
    ...
    if (!(serial != 0)) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 239); };
    size = length;
...
{
    int size;
    if (!(serial != 0)) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 262); };
...
static __inline int _serial_int_rx(struct rt_serial_device *serial, rt_uint8_t *data, int length)
{
    ...
    if (!(serial != 0)) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 293); };
    size = length;

    rx_fifo = (struct rt_serial_rx_fifo*) serial->serial_rx;
    if (!(rx_fifo != 0)) { rt_assert_handler("rx_fifo != RT_NULL", __FUNCTION__, 297); };
...
static __inline int _serial_int_tx(struct rt_serial_device *serial, const rt_uint8_t *data, int length)
{
    ...
    if (!(serial != 0)) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 341); };

    size = length;
    tx = (struct rt_serial_tx_fifo*) serial->serial_tx;
    if (!(tx != 0)) { rt_assert_handler("tx != RT_NULL", __FUNCTION__, 345); };
...
static void _serial_check_buffer_size(void)
{
    ...
    if (already_output == 0)
    {
        ...
        do { rt_kprintf("[" "W" "/" "UART" "] "); rt_kprintf("Warning: There is no enough buffer for saving data," " please increase the RT_SERIAL_RB_BUFSZ option."); rt_kprintf("\n"); } while (0)
                                                                ;
...
static rt_err_t rt_serial_init(struct rt_device *dev)
{
    ...
    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 599); };
...
static rt_err_t rt_serial_open(struct rt_device *dev, rt_uint16_t oflag)
{
    ...
    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 620); };
    serial = (struct rt_serial_device *)dev;
    ...
    if (serial->serial_rx == 0)
    {
        if (oflag & 0x100)
        {
            ...
            rx_fifo = (struct rt_serial_rx_fifo*) rt_malloc (sizeof(struct rt_serial_rx_fifo) +
                serial->config.bufsz);
            if (!(rx_fifo != 0)) { rt_assert_handler("rx_fifo != RT_NULL", __FUNCTION__, 651); };
...
# 692 "../rt-thread/components/drivers/serial/serial.c"
        else
        {
            serial->serial_rx = 0;
        }
    ...
    if (serial->serial_tx == 0)
    {
        if (oflag & 0x400)
        {
            ...
            if (!(tx_fifo != 0)) { rt_assert_handler("tx_fifo != RT_NULL", __FUNCTION__, 714); };
...
# 740 "../rt-thread/components/drivers/serial/serial.c"
        else
        {
            serial->serial_tx = 0;
        }
...
static rt_err_t rt_serial_close(struct rt_device *dev)
{
    ...
    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 765); };
    serial = (struct rt_serial_device *)dev;
    ...
    if (dev->open_flag & 0x100)
    {
        ...
        rx_fifo = (struct rt_serial_rx_fifo*)serial->serial_rx;
        if (!(rx_fifo != 0)) { rt_assert_handler("rx_fifo != RT_NULL", __FUNCTION__, 780); };
...
# 816 "../rt-thread/components/drivers/serial/serial.c"
    if (dev->open_flag & 0x400)
    {
        ...
        if (!(tx_fifo != 0)) { rt_assert_handler("tx_fifo != RT_NULL", __FUNCTION__, 824); };
                                ...
                                rt_size_t size)
{
    ...
    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 864); };
    if (size == 0) return 0;
                                 ...
                                 rt_size_t size)
{
    ...
    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 890); };
    if (size == 0) return 0;
...
# 1006 "../rt-thread/components/drivers/serial/serial.c"
                                  ...
                                  void *args)
{
    ...
    if (!(dev != 0)) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 1013); };
    serial = (struct rt_serial_device *)dev;
                               ...
                               void *data)
{
    ...
    if (!(serial != 0)) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 1287); };
...
void rt_hw_serial_isr(struct rt_serial_device *serial, int event)
{
    switch (event & 0xff)
    {
        case 0x01:
        {
            ...
            rx_fifo = (struct rt_serial_rx_fifo*)serial->serial_rx;
            if (!(rx_fifo != 0)) { rt_assert_handler("rx_fifo != RT_NULL", __FUNCTION__, 1331); };
...
# 1457 "../rt-thread/components/drivers/serial/serial.c"
    }
}
