# 1 "../rt-thread/components/drivers/ipc/waitqueue.c"
...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/drivers/ipc/waitqueue.c"
# 13 "../rt-thread/components/drivers/ipc/waitqueue.c"
...
# 14 "../rt-thread/components/drivers/ipc/waitqueue.c" 2
...
#define rt_hw_dmb() 
#define rt_hw_dsb() 
# 16 "../rt-thread/components/drivers/ipc/waitqueue.c" 2
...
# 17 "../rt-thread/components/drivers/ipc/waitqueue.c" 2
# 26 "../rt-thread/components/drivers/ipc/waitqueue.c"
void rt_wqueue_add(rt_wqueue_t *queue, struct rt_wqueue_node *node)
{
    rt_base_t level;
...
# 59 "../rt-thread/components/drivers/ipc/waitqueue.c"
int __wqueue_default_wake(struct rt_wqueue_node *wait, void *key)
{
    return 0;
}
# 73 "../rt-thread/components/drivers/ipc/waitqueue.c"
void rt_wqueue_wakeup(rt_wqueue_t *queue, void *key)
{
    rt_base_t level;
    int need_schedule = 0;
...
# 121 "../rt-thread/components/drivers/ipc/waitqueue.c"
static int _rt_wqueue_wait(rt_wqueue_t *queue, int condition, int msec, int suspend_flag)
{
    ...
    do { if (1) { rt_bool_t interrupt_disabled; rt_base_t level; interrupt_disabled = rt_hw_interrupt_is_disabled(); level = rt_hw_interrupt_disable(); if (rt_critical_level() != 0) { rt_kprintf("Function[%s]: scheduler is not available\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 131); } } if (interrupt_disabled == 1) { rt_kprintf("Function[%s]: interrupt is disabled\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 131); } } do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_thread_self() == 0) { rt_kprintf("Function[%s] shall not be used before scheduler start\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 131); } } do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_interrupt_get_nest() != 0) { rt_kprintf("Function[%s] shall not be used in ISR\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 131); } } rt_hw_interrupt_enable(level); } while (0); rt_hw_interrupt_enable(level); } while (0); rt_hw_interrupt_enable(level); } } while (0);
