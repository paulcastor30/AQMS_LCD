# 1 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
# 11 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
...
#define rt_hw_dmb() 
#define rt_hw_dsb() 
# 12 "../rt-thread/components/drivers/ipc/ringblk_buf.c" 2
...
# 13 "../rt-thread/components/drivers/ipc/ringblk_buf.c" 2
...
# 14 "../rt-thread/components/drivers/ipc/ringblk_buf.c" 2
# 26 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
void rt_rbb_init(rt_rbb_t rbb, rt_uint8_t *buf, rt_size_t buf_size, rt_rbb_blk_t block_set, rt_size_t blk_max_num)
{
    rt_size_t i;
...
;
# 62 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
rt_rbb_t rt_rbb_create(rt_size_t buf_size, rt_size_t blk_max_num)
{
    rt_rbb_t rbb = 0;
    rt_uint8_t *buf;
...
void rt_rbb_destroy(rt_rbb_t rbb)
{
    if (!(rbb)) { rt_assert_handler("rbb", __FUNCTION__, 102); };
...
static rt_rbb_blk_t find_empty_blk_in_set(rt_rbb_t rbb)
{
    ...
    if (!(rbb)) { rt_assert_handler("rbb", __FUNCTION__, 117); };
    ...
    if (!(blk->status == RT_RBB_BLK_UNUSED)) { rt_assert_handler("blk->status == RT_RBB_BLK_UNUSED", __FUNCTION__, 125); };
    return blk;
}
...
# 168 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
rt_rbb_blk_t rt_rbb_blk_alloc(rt_rbb_t rbb, rt_size_t blk_size)
{
    rt_base_t level;
    ...
    if (!(rbb)) { rt_assert_handler("rbb", __FUNCTION__, 174); };
    if (!(blk_size < (1L << 24))) { rt_assert_handler("blk_size < (1L << 24)", __FUNCTION__, 175); };
...
void rt_rbb_blk_put(rt_rbb_blk_t block)
{
    if (!(block)) { rt_assert_handler("block", __FUNCTION__, 272); };
    if (!(block->status == RT_RBB_BLK_INITED)) { rt_assert_handler("block->status == RT_RBB_BLK_INITED", __FUNCTION__, 273); };

    block->status = RT_RBB_BLK_PUT;
}
;
# 287 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
rt_rbb_blk_t rt_rbb_blk_get(rt_rbb_t rbb)
{
    rt_base_t level;
    ...
    if (!(rbb)) { rt_assert_handler("rbb", __FUNCTION__, 293); };
...
;
# 327 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
rt_size_t rt_rbb_blk_size(rt_rbb_blk_t block)
{
    if (!(block)) { rt_assert_handler("block", __FUNCTION__, 329); };

    return block->size;
}
;
# 342 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
rt_uint8_t *rt_rbb_blk_buf(rt_rbb_blk_t block)
{
    if (!(block)) { rt_assert_handler("block", __FUNCTION__, 344); };
...
void rt_rbb_blk_free(rt_rbb_t rbb, rt_rbb_blk_t block)
{
    ...
    if (!(rbb)) { rt_assert_handler("rbb", __FUNCTION__, 360); };
    if (!(block)) { rt_assert_handler("block", __FUNCTION__, 361); };
    if (!(block->status != RT_RBB_BLK_UNUSED)) { rt_assert_handler("block->status != RT_RBB_BLK_UNUSED", __FUNCTION__, 362); };
...
;
# 396 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
rt_size_t rt_rbb_blk_queue_get(rt_rbb_t rbb, rt_size_t queue_data_len, rt_rbb_blk_queue_t blk_queue)
{
    rt_base_t level;
    ...
    if (!(rbb)) { rt_assert_handler("rbb", __FUNCTION__, 403); };
    if (!(blk_queue)) { rt_assert_handler("blk_queue", __FUNCTION__, 404); };
...
;
# 471 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
rt_size_t rt_rbb_blk_queue_len(rt_rbb_blk_queue_t blk_queue)
{
    rt_size_t i = 0, data_total_size = 0;
    rt_rbb_blk_t blk;

    if (!(blk_queue)) { rt_assert_handler("blk_queue", __FUNCTION__, 476); };
...
;
# 494 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
rt_uint8_t *rt_rbb_blk_queue_buf(rt_rbb_blk_queue_t blk_queue)
{
    if (!(blk_queue)) { rt_assert_handler("blk_queue", __FUNCTION__, 496); };
...
void rt_rbb_blk_queue_free(rt_rbb_t rbb, rt_rbb_blk_queue_t blk_queue)
{
    ...
    if (!(rbb)) { rt_assert_handler("rbb", __FUNCTION__, 513); };
    if (!(blk_queue)) { rt_assert_handler("blk_queue", __FUNCTION__, 514); };
...
;
# 533 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
rt_size_t rt_rbb_next_blk_queue_len(rt_rbb_t rbb)
{
    rt_base_t level;
    ...
    if (!(rbb)) { rt_assert_handler("rbb", __FUNCTION__, 540); };
...
;
# 590 "../rt-thread/components/drivers/ipc/ringblk_buf.c"
rt_size_t rt_rbb_get_buf_size(rt_rbb_t rbb)
{
    if (!(rbb)) { rt_assert_handler("rbb", __FUNCTION__, 592); };
