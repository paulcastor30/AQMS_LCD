# 1 "../rt-thread/components/drivers/ipc/completion.c"
...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/drivers/ipc/completion.c"
# 12 "../rt-thread/components/drivers/ipc/completion.c"
...
#define rt_hw_dmb() 
#define rt_hw_dsb() 
# 13 "../rt-thread/components/drivers/ipc/completion.c" 2
...
# 14 "../rt-thread/components/drivers/ipc/completion.c" 2
...
# 15 "../rt-thread/components/drivers/ipc/completion.c" 2
...
{
    rt_base_t level;
    if (!(completion != 0)) { rt_assert_handler("completion != RT_NULL", __FUNCTION__, 27); };
...
;
# 52 "../rt-thread/components/drivers/ipc/completion.c"
rt_err_t rt_completion_wait(struct rt_completion *completion,
                            rt_int32_t timeout)
{
    rt_err_t result;
    rt_base_t level;
    rt_thread_t thread;
    if (!(completion != 0)) { rt_assert_handler("completion != RT_NULL", __FUNCTION__, 58); };


    do { if (timeout != 0) { rt_bool_t interrupt_disabled; rt_base_t level; interrupt_disabled = rt_hw_interrupt_is_disabled(); level = rt_hw_interrupt_disable(); if (rt_critical_level() != 0) { rt_kprintf("Function[%s]: scheduler is not available\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 61); } } if (interrupt_disabled == 1) { rt_kprintf("Function[%s]: interrupt is disabled\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 61); } } do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_thread_self() == 0) { rt_kprintf("Function[%s] shall not be used before scheduler start\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 61); } } do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_interrupt_get_nest() != 0) { rt_kprintf("Function[%s] shall not be used in ISR\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 61); } } rt_hw_interrupt_enable(level); } while (0); rt_hw_interrupt_enable(level); } while (0); rt_hw_interrupt_enable(level); } } while (0);
    ...
    if (completion->flag != 1)
    {
        ...
        if (!(rt_list_isempty(&(completion->suspended_list)))) { rt_assert_handler("rt_list_isempty(&(completion->suspended_list))", __FUNCTION__, 70); };
...
{
    rt_base_t level;
    if (!(completion != 0)) { rt_assert_handler("completion != RT_NULL", __FUNCTION__, 130); };
