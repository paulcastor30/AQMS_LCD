# 1 "../rt-thread/components/drivers/ipc/workqueue.c"
...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/drivers/ipc/workqueue.c"
# 14 "../rt-thread/components/drivers/ipc/workqueue.c"
...
#define rt_hw_dmb() 
#define rt_hw_dsb() 
# 15 "../rt-thread/components/drivers/ipc/workqueue.c" 2
...
# 16 "../rt-thread/components/drivers/ipc/workqueue.c" 2
...
# 17 "../rt-thread/components/drivers/ipc/workqueue.c" 2
...
static void _workqueue_thread_entry(void *parameter)
{
    ...
    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 60); };
...
# 124 "../rt-thread/components/drivers/ipc/workqueue.c" 3
                    (0xffffffffUL) 
# 124 "../rt-thread/components/drivers/ipc/workqueue.c"
                                / 2)
    {
...
static void _delayed_work_timeout_handler(void *parameter)
{
    ...
    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 178); };
...
# 215 "../rt-thread/components/drivers/ipc/workqueue.c"
void rt_work_init(struct rt_work *work,
                  void (*work_func)(struct rt_work *work, void *work_data),
                  void *work_data)
{
    if (!(work != 0)) { rt_assert_handler("work != RT_NULL", __FUNCTION__, 219); };
    if (!(work_func != 0)) { rt_assert_handler("work_func != RT_NULL", __FUNCTION__, 220); };
...
# 241 "../rt-thread/components/drivers/ipc/workqueue.c"
struct rt_workqueue *rt_workqueue_create(const char *name, rt_uint16_t stack_size, rt_uint8_t priority)
{
...
# 275 "../rt-thread/components/drivers/ipc/workqueue.c"
rt_err_t rt_workqueue_destroy(struct rt_workqueue *queue)
{
    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 277); };
...
# 297 "../rt-thread/components/drivers/ipc/workqueue.c"
rt_err_t rt_workqueue_dowork(struct rt_workqueue *queue, struct rt_work *work)
{
    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 299); };
    if (!(work != 0)) { rt_assert_handler("work != RT_NULL", __FUNCTION__, 300); };

    return _workqueue_submit_work(queue, work, 0);
}
# 320 "../rt-thread/components/drivers/ipc/workqueue.c"
rt_err_t rt_workqueue_submit_work(struct rt_workqueue *queue, struct rt_work *work, rt_tick_t ticks)
{
    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 322); };
    if (!(work != 0)) { rt_assert_handler("work != RT_NULL", __FUNCTION__, 323); };
    if (!(ticks < 
# 324 "../rt-thread/components/drivers/ipc/workqueue.c" 3
   (0xffffffffUL) 
# 324 "../rt-thread/components/drivers/ipc/workqueue.c"
   / 2)) { rt_assert_handler("ticks < RT_TICK_MAX / 2", __FUNCTION__, 324); };

    return _workqueue_submit_work(queue, work, ticks);
}
# 338 "../rt-thread/components/drivers/ipc/workqueue.c"
rt_err_t rt_workqueue_urgent_work(struct rt_workqueue *queue, struct rt_work *work)
{
    rt_base_t level;

    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 342); };
    if (!(work != 0)) { rt_assert_handler("work != RT_NULL", __FUNCTION__, 343); };
...
# 376 "../rt-thread/components/drivers/ipc/workqueue.c"
rt_err_t rt_workqueue_cancel_work(struct rt_workqueue *queue, struct rt_work *work)
{
    if (!(work != 0)) { rt_assert_handler("work != RT_NULL", __FUNCTION__, 378); };
    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 379); };

    return _workqueue_cancel_work(queue, work);
}
# 393 "../rt-thread/components/drivers/ipc/workqueue.c"
rt_err_t rt_workqueue_cancel_work_sync(struct rt_workqueue *queue, struct rt_work *work)
{
    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 395); };
    if (!(work != 0)) { rt_assert_handler("work != RT_NULL", __FUNCTION__, 396); };
...
# 418 "../rt-thread/components/drivers/ipc/workqueue.c"
rt_err_t rt_workqueue_cancel_all_work(struct rt_workqueue *queue)
{
    struct rt_work *work;

    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 422); };
...
# 459 "../rt-thread/components/drivers/ipc/workqueue.c"
rt_err_t rt_work_submit(struct rt_work *work, rt_tick_t ticks)
{
    return rt_workqueue_submit_work(sys_workq, work, ticks);
}
# 471 "../rt-thread/components/drivers/ipc/workqueue.c"
rt_err_t rt_work_urgent(struct rt_work *work)
{
    return rt_workqueue_urgent_work(sys_workq, work);
}
# 484 "../rt-thread/components/drivers/ipc/workqueue.c"
rt_err_t rt_work_cancel(struct rt_work *work)
{
...
static int rt_work_sys_workqueue_init(void)
{
    ...
    sys_workq = rt_workqueue_create("sys workq", 2048,
                                    23);
    if (!(sys_workq != 0)) { rt_assert_handler("sys_workq != RT_NULL", __FUNCTION__, 496); };

    return 0;
}
__attribute__((used)) const init_fn_t __rt_init_rt_work_sys_workqueue_init __attribute__((section(".rti_fn." "2"))) = rt_work_sys_workqueue_init;
