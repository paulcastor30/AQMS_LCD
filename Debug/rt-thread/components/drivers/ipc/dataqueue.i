# 1 "../rt-thread/components/drivers/ipc/dataqueue.c"
...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/drivers/ipc/dataqueue.c"
# 12 "../rt-thread/components/drivers/ipc/dataqueue.c"
...
# 13 "../rt-thread/components/drivers/ipc/dataqueue.c" 2
...
# 14 "../rt-thread/components/drivers/ipc/dataqueue.c" 2
...
# 41 "../rt-thread/components/drivers/ipc/dataqueue.c"
rt_err_t
rt_data_queue_init(struct rt_data_queue *queue,
                   ...
                   void (*evt_notify)(struct rt_data_queue *queue, rt_uint32_t event))
{
    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 47); };
    if (!(size > 0)) { rt_assert_handler("size > 0", __FUNCTION__, 48); };
...
;
# 89 "../rt-thread/components/drivers/ipc/dataqueue.c"
rt_err_t rt_data_queue_push(struct rt_data_queue *queue,
                            const void *data_ptr,
                            rt_size_t data_size,
                            rt_int32_t timeout)
{
    ...
    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 98); };
    if (!(queue->magic == 0xbead0e0e)) { rt_assert_handler("queue->magic == DATAQUEUE_MAGIC", __FUNCTION__, 99); };


    do { if (timeout != 0) { rt_bool_t interrupt_disabled; rt_base_t level; interrupt_disabled = rt_hw_interrupt_is_disabled(); level = rt_hw_interrupt_disable(); if (rt_critical_level() != 0) { rt_kprintf("Function[%s]: scheduler is not available\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 102); } } if (interrupt_disabled == 1) { rt_kprintf("Function[%s]: interrupt is disabled\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 102); } } do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_thread_self() == 0) { rt_kprintf("Function[%s] shall not be used before scheduler start\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 102); } } do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_interrupt_get_nest() != 0) { rt_kprintf("Function[%s] shall not be used in ISR\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 102); } } rt_hw_interrupt_enable(level); } while (0); rt_hw_interrupt_enable(level); } while (0); rt_hw_interrupt_enable(level); } } while (0);
...
;
# 206 "../rt-thread/components/drivers/ipc/dataqueue.c"
rt_err_t rt_data_queue_pop(struct rt_data_queue *queue,
                           const void **data_ptr,
                           rt_size_t *size,
                           rt_int32_t timeout)
{
    ...
    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 215); };
    if (!(queue->magic == 0xbead0e0e)) { rt_assert_handler("queue->magic == DATAQUEUE_MAGIC", __FUNCTION__, 216); };
    if (!(data_ptr != 0)) { rt_assert_handler("data_ptr != RT_NULL", __FUNCTION__, 217); };
    if (!(size != 0)) { rt_assert_handler("size != RT_NULL", __FUNCTION__, 218); };


    do { if (timeout != 0) { rt_bool_t interrupt_disabled; rt_base_t level; interrupt_disabled = rt_hw_interrupt_is_disabled(); level = rt_hw_interrupt_disable(); if (rt_critical_level() != 0) { rt_kprintf("Function[%s]: scheduler is not available\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 221); } } if (interrupt_disabled == 1) { rt_kprintf("Function[%s]: interrupt is disabled\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 221); } } do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_thread_self() == 0) { rt_kprintf("Function[%s] shall not be used before scheduler start\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 221); } } do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_interrupt_get_nest() != 0) { rt_kprintf("Function[%s] shall not be used in ISR\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 221); } } rt_hw_interrupt_enable(level); } while (0); rt_hw_interrupt_enable(level); } while (0); rt_hw_interrupt_enable(level); } } while (0);
...
;
# 329 "../rt-thread/components/drivers/ipc/dataqueue.c"
rt_err_t rt_data_queue_peek(struct rt_data_queue *queue,
                            const void **data_ptr,
                            rt_size_t *size)
{
    ...
    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 335); };
    if (!(queue->magic == 0xbead0e0e)) { rt_assert_handler("queue->magic == DATAQUEUE_MAGIC", __FUNCTION__, 336); };
...
;
# 362 "../rt-thread/components/drivers/ipc/dataqueue.c"
void rt_data_queue_reset(struct rt_data_queue *queue)
{
    rt_base_t level;
    struct rt_thread *thread;

    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 367); };
    if (!(queue->magic == 0xbead0e0e)) { rt_assert_handler("queue->magic == DATAQUEUE_MAGIC", __FUNCTION__, 368); };
...
;
# 442 "../rt-thread/components/drivers/ipc/dataqueue.c"
rt_err_t rt_data_queue_deinit(struct rt_data_queue *queue)
{
    rt_base_t level;

    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 446); };
    if (!(queue->magic == 0xbead0e0e)) { rt_assert_handler("queue->magic == DATAQUEUE_MAGIC", __FUNCTION__, 447); };
...
;
# 469 "../rt-thread/components/drivers/ipc/dataqueue.c"
rt_uint16_t rt_data_queue_len(struct rt_data_queue *queue)
{
    rt_base_t level;
    rt_int16_t len;

    if (!(queue != 0)) { rt_assert_handler("queue != RT_NULL", __FUNCTION__, 474); };
    if (!(queue->magic == 0xbead0e0e)) { rt_assert_handler("queue->magic == DATAQUEUE_MAGIC", __FUNCTION__, 475); };
