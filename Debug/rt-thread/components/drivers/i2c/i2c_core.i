...
# 13 "../rt-thread/components/drivers/i2c/i2c_core.c" 2
...
# 21 "../rt-thread/components/drivers/i2c/i2c_core.c" 2
                                    ...
                                    const char *bus_name)
{
    ...
    do { rt_kprintf("[" "I" "/" "I2C" "] "); rt_kprintf("I2C bus [%s] registered", bus_name); rt_kprintf("\n"); } while (0);
...
struct rt_i2c_bus_device *rt_i2c_bus_device_find(const char *bus_name)
{
    ...
    {
        do { rt_kprintf("[" "E" "/" "I2C" "] "); rt_kprintf("I2C bus %s not exist", bus_name); rt_kprintf("\n"); } while (0);
                          ...
                          rt_uint32_t num)
{
    ...
    if (bus->ops->master_xfer)
    {
        ...
        {
            do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("msgs[%d] %c, addr=0x%02x, len=%d", ret, (msgs[ret].flags & (1u << 0)) ? 'R' : 'W', msgs[ret].addr, msgs[ret].len); rt_kprintf("\n"); } while (0)
    ...
    {
        do { rt_kprintf("[" "E" "/" "I2C" "] "); rt_kprintf("I2C bus operation not supported"); rt_kprintf("\n"); } while (0);
        return -10;
    }
}
                             ...
                             rt_uint32_t count)
{
    rt_ssize_t ret;
    struct rt_i2c_msg msg;
    if (!(bus != 0)) { rt_assert_handler("bus != RT_NULL", __FUNCTION__, 136); };
