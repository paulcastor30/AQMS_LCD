...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/drivers/i2c/i2c-bit-ops.c"
# 11 "../rt-thread/components/drivers/i2c/i2c-bit-ops.c"
...
# 12 "../rt-thread/components/drivers/i2c/i2c-bit-ops.c" 2
...
# 20 "../rt-thread/components/drivers/i2c/i2c-bit-ops.c" 2
...
static rt_err_t SCL_H(struct rt_i2c_bit_ops *ops)
{
    ...
    {
        do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("wait %ld tick for SCL line to go high", rt_tick_get() - start); rt_kprintf("\n"); } while (0)
                                    ;
    }
...
static void i2c_start(struct rt_i2c_bit_ops *ops)
{
    ...
    if (ops->get_scl && !ops->get_scl(ops->data))
    {
        do { rt_kprintf("[" "E" "/" "I2C" "] "); rt_kprintf("I2C bus error, SCL line low"); rt_kprintf("\n"); } while (0);
    }
    if (ops->get_sda && !ops->get_sda(ops->data))
    {
        do { rt_kprintf("[" "E" "/" "I2C" "] "); rt_kprintf("I2C bus error, SDA line low"); rt_kprintf("\n"); } while (0);
    }
...
static __inline rt_bool_t i2c_waitack(struct rt_i2c_bit_ops *ops)
{
    ...
    if (SCL_H(ops) < 0)
    {
        do { rt_kprintf("[" "W" "/" "I2C" "] "); rt_kprintf("wait ack timeout"); rt_kprintf("\n"); } while (0);

        return -2;
    }

    ack = !ops->get_sda(ops->data);
    do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("%s", ack ? "ACK" : "NACK"); rt_kprintf("\n"); } while (0);
...
static rt_int32_t i2c_writeb(struct rt_i2c_bus_device *bus, rt_uint8_t data)
{
    ...
    for (i = 7; i >= 0; i--)
    {
        ...
        if (SCL_H(ops) < 0)
        {
            do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("i2c_writeb: 0x%02x, " "wait scl pin high timeout at bit %d", data, i); rt_kprintf("\n"); } while (0)
...
static rt_int32_t i2c_readb(struct rt_i2c_bus_device *bus)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        if (SCL_H(ops) < 0)
        {
            do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("i2c_readb: wait scl pin high " "timeout at bit %d", 7 - i); rt_kprintf("\n"); } while (0)
                                               ;
                                ...
                                struct rt_i2c_msg *msg)
{
    ...
    while (count > 0)
    {
        ...
        {
            do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("send bytes: NACK."); rt_kprintf("\n"); } while (0);

            return 0;
        }
        else
        {
            do { rt_kprintf("[" "E" "/" "I2C" "] "); rt_kprintf("send bytes: error %d", ret); rt_kprintf("\n"); } while (0);
...
static rt_err_t i2c_send_ack_or_nack(struct rt_i2c_bus_device *bus, int ack)
{
    ...
    if (SCL_H(ops) < 0)
    {
        do { rt_kprintf("[" "E" "/" "I2C" "] "); rt_kprintf("ACK or NACK timeout."); rt_kprintf("\n"); } while (0);
                                ...
                                struct rt_i2c_msg *msg)
{
    ...
    while (count > 0)
    {
        ...
        do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("recieve bytes: 0x%02x, %s", val, (flags & (1u << 6)) ? "(No ACK/NACK)" : (count ? "ACK" : "NACK")); rt_kprintf("\n"); } while (0)
                                   ...
                                   rt_int32_t retries)
{
    ...
    for (i = 0; i <= retries; i++)
    {
        ...
        if (ret == 1 || i == retries)
            break;
        do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("send stop condition"); rt_kprintf("\n"); } while (0);
        i2c_stop(ops);
        i2c_delay2(ops);
        do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("send start condition"); rt_kprintf("\n"); } while (0);
        i2c_start(ops);
    }
                                     ...
                                     struct rt_i2c_msg *msg)
{
    ...
    if (flags & (1u << 2))
    {
        ...
        do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("addr1: %d, addr2: %d", addr1, addr2); rt_kprintf("\n"); } while (0);

        ret = i2c_send_address(bus, addr1, retries);
        if ((ret != 1) && !ignore_nack)
        {
            do { rt_kprintf("[" "W" "/" "I2C" "] "); rt_kprintf("NACK: sending first addr"); rt_kprintf("\n"); } while (0);
        ...
        if ((ret != 1) && !ignore_nack)
        {
            do { rt_kprintf("[" "W" "/" "I2C" "] "); rt_kprintf("NACK: sending second addr"); rt_kprintf("\n"); } while (0);

            return -8;
        }
        if (flags & (1u << 0))
        {
            do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("send repeated start condition"); rt_kprintf("\n"); } while (0);
            i2c_restart(ops);
            addr1 |= 0x01;
            ret = i2c_send_address(bus, addr1, retries);
            if ((ret != 1) && !ignore_nack)
            {
                do { rt_kprintf("[" "E" "/" "I2C" "] "); rt_kprintf("NACK: sending repeated addr"); rt_kprintf("\n"); } while (0);
                              ...
                              rt_uint32_t num)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (!(msg->flags & (1u << 4)))
        {
            ...
            {
                do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("receive NACK from device addr 0x%02x msg %d", msgs[i].addr, i); rt_kprintf("\n"); } while (0)
                                        ;
                goto out;
            }
        }
        if (msg->flags & (1u << 0))
        {
            ...
            if (ret >= 1)
            {
                do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("read %d byte%s", ret, ret == 1 ? "" : "s"); rt_kprintf("\n"); } while (0);
            }
            if (ret < msg->len)
            {
                if (ret >= 0)
        ...
        else
        {
            ...
            if (ret >= 1)
            {
                do { rt_kprintf("[" "D" "/" "I2C" "] "); rt_kprintf("write %d byte%s", ret, ret == 1 ? "" : "s"); rt_kprintf("\n"); } while (0);
            }
            if (ret < msg->len)
            {
                if (ret >= 0)
