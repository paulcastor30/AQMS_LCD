# 1 "../rt-thread/components/net/sal/src/sal_socket.c"
...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/net/sal/src/sal_socket.c"
# 12 "../rt-thread/components/net/sal/src/sal_socket.c"
...
# 250 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD/rtconfig.h"
...
# 296 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD/rtconfig.h"
...
# 13 "../rt-thread/components/net/sal/src/sal_socket.c" 2
...
# 15 "../rt-thread/components/net/sal/src/sal_socket.c" 2
# 1 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD\\rt-thread\\components\\libc\\compilers\\common\\include/sys/ioctl.h" 1
# 12 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD\\rt-thread\\components\\libc\\compilers\\common\\include/sys/ioctl.h"
...
# 35 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD\\rt-thread\\components\\libc\\compilers\\common\\include/sys/ioctl.h"
#define _IOC_NONE 0U
...
# 16 "../rt-thread/components/net/sal/src/sal_socket.c" 2
...
# 18 "../rt-thread/components/net/sal/src/sal_socket.c" 2
...
int sal_getaddrinfo(const char *nodename,
       ...
       struct addrinfo **res);
# 19 "../rt-thread/components/net/sal/src/sal_socket.c" 2



# 1 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD\\rt-thread\\components\\net\\sal\\include/sal_low_lvl.h" 1
...
#define __RTC_H__ 
# 24 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD\\rt-thread\\components\\drivers\\include/drivers/rtc.h"
...
# 40 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD\\rt-thread\\components\\net\\sal\\include/sal_low_lvl.h"
...
struct sal_proto_family
{
    ...
    const struct sal_netdb_ops *netdb_ops;
};
...
# 23 "../rt-thread/components/net/sal/src/sal_socket.c" 2
...
# 24 "../rt-thread/components/net/sal/src/sal_socket.c" 2


# 1 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD\\rt-thread\\components\\drivers\\include/ipc/workqueue.h" 1
# 27 "../rt-thread/components/net/sal/src/sal_socket.c" 2
...
# 37 "../rt-thread/components/net/sal/src/sal_socket.c" 2
...
static rt_bool_t init_ok = 0;
static struct sal_netdev_res_table sal_dev_res_tbl[16];

#define IS_SOCKET_PROTO_TLS(sock) (((sock)->protocol == PROTOCOL_TLS) || ((sock)->protocol == PROTOCOL_DTLS))

#define SAL_SOCKOPS_PROTO_TLS_VALID(sock,name) (proto_tls && (proto_tls->ops->name) && IS_SOCKET_PROTO_TLS(sock))

#define SAL_SOCKOPT_PROTO_TLS_EXEC(sock,name,optval,optlen) do { if (SAL_SOCKOPS_PROTO_TLS_VALID(sock, name)){ return proto_tls->ops->name((sock)->user_data_tls, (optval), (optlen)); } }while(0)
...
#define SAL_SOCKET_OBJ_GET(sock,socket) do { (sock) = sal_get_socket(socket); if ((sock) == RT_NULL) { return -1; } }while(0)
...
#define SAL_NETDEV_IS_UP(netdev) do { if (!netdev_is_up(netdev)) { return -1; } }while(0)
...
#define SAL_NETDEV_SOCKETOPS_VALID(netdev,pf,ops) do { (pf) = (struct sal_proto_family *) netdev->sal_user_data; if ((pf)->skt_ops->ops == RT_NULL){ return -1; } }while(0)
...
#define SAL_NETDEV_NETDBOPS_VALID(netdev,pf,ops) ((netdev) && netdev_is_up(netdev) && ((pf) = (struct sal_proto_family *) (netdev)->sal_user_data) != RT_NULL && (pf)->netdb_ops->ops)




#define SAL_NETDBOPS_VALID(netdev,pf,ops) ((netdev) && ((pf) = (struct sal_proto_family *) (netdev)->sal_user_data) != RT_NULL && (pf)->netdb_ops->ops)
# 115 "../rt-thread/components/net/sal/src/sal_socket.c"
int sal_init(void)
{
    int cn;
    ...
    if (socket_table.sockets == 0)
    {
        do { rt_kprintf("[" "E" "/" "sal.skt" "] "); rt_kprintf("No memory for socket table.\n"); rt_kprintf("\n"); } while (0);
        return -1;
    }
    ...
    rt_mutex_init(&sal_core_lock, "sal_lock", 0x01);

    do { rt_kprintf("[" "I" "/" "sal.skt" "] "); rt_kprintf("Socket Abstraction Layer initialize success."); rt_kprintf("\n"); } while (0);
    init_ok = 1;

    return 0;
}
__attribute__((used)) const init_fn_t __rt_init_sal_init __attribute__((section(".rti_fn." "4"))) = sal_init;
...
#define SAL_INTERNET_DATE_LEN 16
    ...
    if (pf == 0 || pf->skt_ops == 0)
    {
        result = -1;
        goto __exit;
    }

    host = (struct hostent *) pf->netdb_ops->gethostbyname("link.rt-thread.org");
    if (host == 0)
    {
        result = -1;
        goto __exit;
    ...
    skt_ops = pf->skt_ops;
    if ((sockfd = skt_ops->socket(2, 2, 0)) < 0)
    {
        result = -1;
        goto __exit;
    }
    ...
    server_addr.sin_port = (uint16_t)((((8101) & 0x00ffUL) << 8) | (((8101) & 0xff00UL) >> 8));
    ...
    skt_ops->setsockopt(sockfd, 0xfff, 0x1006, (void *) &timeout, sizeof(timeout));
    skt_ops->setsockopt(sockfd, 0xfff, 0x1005, (void *) &timeout, sizeof(timeout));


    rt_memset(date, 0x00, 16);
    rt_snprintf(date, 16, "%s", "Jul 20 2023");

    for (index = 0; index < sizeof(month) / 4; index++)
    {
        if (rt_memcmp(date, month[index], 4 - 1) == 0)
        {
            moth_num = index + 1;
            break;
        }
    ...
    if (moth_num == 0 || moth_num > sizeof(month) / 4)
    {
        result = -1;
        goto __exit;
    }

    rt_memset(send_data, 0x00, 12);
    send_data[0] = 0x00;
    ...
    skt_ops->sendto(sockfd, send_data, 12, 0,
...
__exit:
    ...
    {
        ;
        netdev_low_level_set_internet_status(netdev, 1);
    }
    else
    {
        ;
        netdev_low_level_set_internet_status(netdev, 0);
    }
...
int sal_check_netdev_internet_up(struct netdev *netdev)
{
    if (!(netdev)) { rt_assert_handler("netdev", __FUNCTION__, 287); };
    ...
    if (net_work == 0)
    {
        do { rt_kprintf("[" "W" "/" "sal.skt" "] "); rt_kprintf("No memory for network interface device(%s) delay work.", netdev->name); rt_kprintf("\n"); } while (0);
        return -1;
    }
...
# 331 "../rt-thread/components/net/sal/src/sal_socket.c"
struct sal_socket *sal_get_socket(int socket)
{
    ...
    if (!(st->sockets[socket]->magic == 0x5A10)) { rt_assert_handler("st->sockets[socket]->magic == SAL_SOCKET_MAGIC", __FUNCTION__, 343); };
...
static void sal_lock(void)
{
    ...
    result = rt_mutex_take(&sal_core_lock, -1);
    if (result != 0)
    {
        if (!(0)) { rt_assert_handler("0", __FUNCTION__, 360); };
    }
}
...
# 420 "../rt-thread/components/net/sal/src/sal_socket.c"
static int socket_init(int family, int type, int protocol, struct sal_socket **res)
{
    ...
    if (family < 0 || family > (46 + 1))
    {
        return -1;
    }

    if (type < 0 || type > (02000000 + 1))
    {
        return -2;
    }
    ...
    if (netdv_def && (((netdv_def)->flags & 0x01U) ? (uint8_t)1 : (uint8_t)0))
    {

        pf = (struct sal_proto_family *) netdv_def->sal_user_data;
        if (pf != 0 && pf->skt_ops && (pf->family == family || pf->sec_family == family))
        {
    ...
    if (flag == 0)
    {
        ...
        if (netdev == 0)
        {
            do { rt_kprintf("[" "E" "/" "sal.skt" "] "); rt_kprintf("not find network interface device by protocol family(%d).", family); rt_kprintf("\n"); } while (0);
            return -3;
        }
...
static int socket_alloc(struct sal_socket_table *st, int f_socket)
{
    ...
    if (idx == (int) st->max_socket && st->max_socket < 16)
    {
        int cnt, index;
        struct sal_socket **sockets;
    ...
    if (idx < (int) st->max_socket && st->sockets[idx] == 0)
    {
...
static void socket_delete(int socket)
{
    ...
    if (!(sock != 0)) { rt_assert_handler("sock != RT_NULL", __FUNCTION__, 576); };
    sock->magic = 0;
    sock->netdev = 0;
    socket_free(st, idx);
    sal_unlock();
...
int sal_accept(int socket, struct sockaddr *addr, socklen_t *addrlen)
{
    ...
    do { (sock) = sal_get_socket(socket); if ((sock) == 0) { return -1; } }while(0);


    do { if (!(((sock->netdev)->flags & 0x01U) ? (uint8_t)1 : (uint8_t)0)) { return -1; } }while(0);


    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->accept == 0){ return -1; } }while(0);
    ...
    if (new_socket != -1)
    {
        ...
        if (retval < 0)
        {
            ...
            do { rt_kprintf("[" "E" "/" "sal.skt" "] "); rt_kprintf("New socket registered failed, return error %d.", retval); rt_kprintf("\n"); } while (0);
            return -1;
        }
...
int sal_bind(int socket, const struct sockaddr *name, socklen_t namelen)
{
    ...
    if (!(name)) { rt_assert_handler("name", __FUNCTION__, 656); };
    ...
    if (!((input_ipaddr).addr == ((uint32_t)0x00000000UL)))
    {
        struct sal_proto_family *input_pf = 0, *local_pf = 0;
        ...
        do { (local_pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((local_pf)->skt_ops->bind == 0){ return -1; } }while(0);
        do { (input_pf) = (struct sal_proto_family *) new_netdev->sal_user_data; if ((input_pf)->skt_ops->bind == 0){ return -1; } }while(0);
    ...
    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->bind == 0){ return -1; } }while(0);
...
int sal_shutdown(int socket, int how)
{
    ...
    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->shutdown == 0){ return -1; } }while(0);

    if (pf->skt_ops->shutdown((int)(size_t)sock->user_data, how) == 0)
    {
# 727 "../rt-thread/components/net/sal/src/sal_socket.c"
        error = 0;
    }
    else
    {
...
int sal_getpeername(int socket, struct sockaddr *name, socklen_t *namelen)
{
    ...
    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->getpeername == 0){ return -1; } }while(0);
...
int sal_getsockname(int socket, struct sockaddr *name, socklen_t *namelen)
{
    ...
    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->getsockname == 0){ return -1; } }while(0);
...
int sal_getsockopt(int socket, int level, int optname, void *optval, socklen_t *optlen)
{
    ...
    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->getsockopt == 0){ return -1; } }while(0);
...
int sal_setsockopt(int socket, int level, int optname, const void *optval, socklen_t optlen)
{
    ...
    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->setsockopt == 0){ return -1; } }while(0);
# 823 "../rt-thread/components/net/sal/src/sal_socket.c"
...
int sal_connect(int socket, const struct sockaddr *name, socklen_t namelen)
{
    ...
    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->connect == 0){ return -1; } }while(0);

    ret = pf->skt_ops->connect((int)(size_t)sock->user_data, name, namelen);
# 854 "../rt-thread/components/net/sal/src/sal_socket.c"
    return ret;
}
...
int sal_listen(int socket, int backlog)
{
    ...
    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->listen == 0){ return -1; } }while(0);
                 ...
                 struct sockaddr *from, socklen_t *fromlen)
{
    ...
    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->recvfrom == 0){ return -1; } }while(0);
# 901 "../rt-thread/components/net/sal/src/sal_socket.c"
               ...
               const struct sockaddr *to, socklen_t tolen)
{
    ...
    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->sendto == 0){ return -1; } }while(0);
# 935 "../rt-thread/components/net/sal/src/sal_socket.c"
...
int sal_socket(int domain, int type, int protocol)
{
    ...
    if (retval < 0)
    {
        do { rt_kprintf("[" "E" "/" "sal.skt" "] "); rt_kprintf("SAL socket protocol family input failed, return error %d.", retval); rt_kprintf("\n"); } while (0);
        socket_delete(socket);
        return -1;
    }


    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->socket == 0){ return -1; } }while(0);

    proto_socket = pf->skt_ops->socket(domain, type, protocol);
    if (proto_socket >= 0)
    {
# 987 "../rt-thread/components/net/sal/src/sal_socket.c"
...
int sal_closesocket(int socket)
{
    ...
    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->closesocket == 0){ return -1; } }while(0);

    if (pf->skt_ops->closesocket((int)(size_t)sock->user_data) == 0)
    {
# 1018 "../rt-thread/components/net/sal/src/sal_socket.c"
        error = 0;
    }
    else
    {
...
int sal_ioctlsocket(int socket, long cmd, void *arg)
{
    ...
    do { (pf) = (struct sal_proto_family *) sock->netdev->sal_user_data; if ((pf)->skt_ops->ioctlsocket == 0){ return -1; } }while(0);

    struct sal_ifreq *ifr = (struct sal_ifreq *)arg;

    if((sock->domain == 2)&&(sock->netdev)&&(ifr != 0))
    {
        switch (cmd)
        {
        case 0x8915:
...
# 1119 "../rt-thread/components/net/sal/src/sal_socket.c"
struct hostent *sal_gethostbyname(const char *name)
{
    struct netdev *netdev = netdev_default;
    struct sal_proto_family *pf;

    if (((netdev) && (((netdev)->flags & 0x01U) ? (uint8_t)1 : (uint8_t)0) && ((pf) = (struct sal_proto_family *) (netdev)->sal_user_data) != 0 && (pf)->netdb_ops->gethostbyname))
    {
    ...
    else
    {
        ...
        netdev = netdev_get_first_by_flags(0x01U);
        if (((netdev) && (((netdev)->flags & 0x01U) ? (uint8_t)1 : (uint8_t)0) && ((pf) = (struct sal_proto_family *) (netdev)->sal_user_data) != 0 && (pf)->netdb_ops->gethostbyname))
        {
            return pf->netdb_ops->gethostbyname(name);
        }
                        ...
                        size_t buflen, struct hostent **result, int *h_errnop)
{
    ...
    if (((netdev) && (((netdev)->flags & 0x01U) ? (uint8_t)1 : (uint8_t)0) && ((pf) = (struct sal_proto_family *) (netdev)->sal_user_data) != 0 && (pf)->netdb_ops->gethostbyname_r))
    {
                    ...
                    struct addrinfo **res)
{
    ...
    if (((netdev) && (((netdev)->flags & 0x01U) ? (uint8_t)1 : (uint8_t)0) && ((pf) = (struct sal_proto_family *) (netdev)->sal_user_data) != 0 && (pf)->netdb_ops->getaddrinfo))
    {
    ...
    if(ret == 0)
    {
        ...
        if (!((i < 16))) { rt_assert_handler("(i < SAL_SOCKETS_NUM)", __FUNCTION__, 1205); };
...
void sal_freeaddrinfo(struct addrinfo *ai)
{
    ...
    for(i = 0; i < 16; i++)
    {
        if(sal_dev_res_tbl[i].res == ai)
        {
            ...
            sal_dev_res_tbl[i].res = 0;
            sal_dev_res_tbl[i].netdev = 0;
            break;
        }
    }
    if (!((i < 16))) { rt_assert_handler("(i < SAL_SOCKETS_NUM)", __FUNCTION__, 1229); };

    if (((netdev) && ((pf) = (struct sal_proto_family *) (netdev)->sal_user_data) != 0 && (pf)->netdb_ops->freeaddrinfo))
    {
