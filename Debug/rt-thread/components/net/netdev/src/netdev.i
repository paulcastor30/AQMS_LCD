# 1 "../rt-thread/components/net/netdev/src/netdev.c"
...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/net/netdev/src/netdev.c"
# 11 "../rt-thread/components/net/netdev/src/netdev.c"
...
# 12 "../rt-thread/components/net/netdev/src/netdev.c" 2
...
# 13 "../rt-thread/components/net/netdev/src/netdev.c" 2
...
# 14 "../rt-thread/components/net/netdev/src/netdev.c" 2
...
# 16 "../rt-thread/components/net/netdev/src/netdev.c" 2
...
# 19 "../rt-thread/components/net/netdev/src/netdev.c" 2
...
# 20 "../rt-thread/components/net/netdev/src/netdev.c" 2
...
# 29 "../rt-thread/components/net/netdev/src/netdev.c" 2


struct netdev *netdev_list = 0;

struct netdev *netdev_default = 0;

static netdev_callback_fn g_netdev_register_callback = 0;
static netdev_callback_fn g_netdev_default_change_callback = 0;
# 49 "../rt-thread/components/net/netdev/src/netdev.c"
int netdev_register(struct netdev *netdev, const char *name, void *user_data)
{
    rt_base_t level;
    ...
    if (!(netdev)) { rt_assert_handler("netdev", __FUNCTION__, 55); };
    if (!(name)) { rt_assert_handler("name", __FUNCTION__, 56); };


    flags_mask = 0x01U | 0x04U | 0x80U | 0x100U;
    netdev->flags &= ~flags_mask;

    ((&(netdev->ip_addr))->addr = 0);
    ((&(netdev->netmask))->addr = 0);
    ((&(netdev->gw))->addr = 0);
    ...
    {
        ((&(netdev->dns_servers[index]))->addr = 0);
        ;
    }
    netdev->status_callback = 0;
    netdev->addr_callback = 0;

    if(rt_strlen(name) > 8)
    {
        char netdev_name[8 + 1] = {0};

        rt_strncpy(netdev_name, name, 8);
        do { rt_kprintf("[" "E" "/" "netdev" "] "); rt_kprintf("netdev name[%s] length is so long that have been cut into [%s].", name, netdev_name); rt_kprintf("\n"); } while (0);
    }


    rt_strncpy(netdev->name, name, 8);
...
# 136 "../rt-thread/components/net/netdev/src/netdev.c"
int netdev_unregister(struct netdev *netdev)
{
    rt_base_t level;
    rt_slist_t *node = 0;
    struct netdev *cur_netdev = 0;

    if (!(netdev)) { rt_assert_handler("netdev", __FUNCTION__, 142); };
    ...
    {
        cur_netdev = ((struct netdev *)((char *)(node) - (unsigned long)(&((struct netdev *)0)->list)));
        if (cur_netdev == netdev)
        {

            if (netdev_list == netdev && rt_slist_next(&netdev_list->list) == 0)
            {
                netdev_list = 0;
            }
            else
...
# 211 "../rt-thread/components/net/netdev/src/netdev.c"
struct netdev *netdev_get_first_by_flags(uint16_t flags)
{
    rt_base_t level;
    rt_slist_t *node = 0;
    ...
    for (node = &(netdev_list->list); node; node = rt_slist_next(node))
    {
        netdev = ((struct netdev *)((char *)(node) - (unsigned long)(&((struct netdev *)0)->list)));
...
# 248 "../rt-thread/components/net/netdev/src/netdev.c"
struct netdev *netdev_get_by_ipaddr(ip_addr_t *ip_addr)
{
    rt_base_t level;
    rt_slist_t *node = 0;
    ...
    {
        netdev = ((struct netdev *)((char *)(node) - (unsigned long)(&((struct netdev *)0)->list)));
        if (netdev && ((&(netdev->ip_addr))->addr == (ip_addr)->addr))
        {
            rt_hw_interrupt_enable(level);
            return netdev;
        }
...
# 285 "../rt-thread/components/net/netdev/src/netdev.c"
struct netdev *netdev_get_by_name(const char *name)
{
    rt_base_t level;
    rt_slist_t *node = 0;
    ...
    {
        netdev = ((struct netdev *)((char *)(node) - (unsigned long)(&((struct netdev *)0)->list)));
        if (netdev && (rt_strncmp(netdev->name, name, rt_strlen(netdev->name) < 8 ? rt_strlen(netdev->name) : 8) == 0))
        {
            rt_hw_interrupt_enable(level);
            return netdev;
        }
...
# 385 "../rt-thread/components/net/netdev/src/netdev.c"
void netdev_set_default(struct netdev *netdev)
{
...
# 425 "../rt-thread/components/net/netdev/src/netdev.c"
int netdev_set_up(struct netdev *netdev)
{
    if (!(netdev)) { rt_assert_handler("netdev", __FUNCTION__, 427); };

    if (!netdev->ops || !netdev->ops->set_up)
    {
        do { rt_kprintf("[" "E" "/" "netdev" "] "); rt_kprintf("The network interface device(%s) not support to set status.", netdev->name); rt_kprintf("\n"); } while (0);
        return -1;
    }


    if ((((netdev)->flags & 0x01U) ? (uint8_t)1 : (uint8_t)0))
    {
        return 0;
    }
...
# 452 "../rt-thread/components/net/netdev/src/netdev.c"
int netdev_set_down(struct netdev *netdev)
{
    if (!(netdev)) { rt_assert_handler("netdev", __FUNCTION__, 454); };
...
# 481 "../rt-thread/components/net/netdev/src/netdev.c"
int netdev_dhcp_enabled(struct netdev *netdev, rt_bool_t is_enabled)
{
    if (!(netdev)) { rt_assert_handler("netdev", __FUNCTION__, 483); };

    if (!netdev->ops || !netdev->ops->set_dhcp)
    {
        do { rt_kprintf("[" "E" "/" "netdev" "] "); rt_kprintf("The network interface device(%s) not support to set DHCP status.", netdev->name); rt_kprintf("\n"); } while (0);
        return -1;
    }


    if ((((netdev)->flags & 0x100U) ? (uint8_t)1 : (uint8_t)0) == is_enabled)
    {
        return 0;
    }
...
# 510 "../rt-thread/components/net/netdev/src/netdev.c"
int netdev_set_ipaddr(struct netdev *netdev, const ip_addr_t *ip_addr)
{
    if (!(netdev)) { rt_assert_handler("netdev", __FUNCTION__, 512); };
    if (!(ip_addr)) { rt_assert_handler("ip_addr", __FUNCTION__, 513); };

    if (!netdev->ops || !netdev->ops->set_addr_info)
    {
        do { rt_kprintf("[" "E" "/" "netdev" "] "); rt_kprintf("The network interface device(%s) not support to set IP address.", netdev->name); rt_kprintf("\n"); } while (0);
        return -1;
    }

    if ((((netdev)->flags & 0x100U) ? (uint8_t)1 : (uint8_t)0))
    {
        do { rt_kprintf("[" "E" "/" "netdev" "] "); rt_kprintf("The network interface device(%s) DHCP capability is enable, not support set IP address.", netdev->name); rt_kprintf("\n"); } while (0);
        return -1;
    }


    return netdev->ops->set_addr_info(netdev, (ip_addr_t *)ip_addr, 0, 0);
}
# 540 "../rt-thread/components/net/netdev/src/netdev.c"
int netdev_set_netmask(struct netdev *netdev, const ip_addr_t *netmask)
{
    if (!(netdev)) { rt_assert_handler("netdev", __FUNCTION__, 542); };
    if (!(netmask)) { rt_assert_handler("netmask", __FUNCTION__, 543); };

    if (!netdev->ops || !netdev->ops->set_addr_info)
    {
        do { rt_kprintf("[" "E" "/" "netdev" "] "); rt_kprintf("The network interface device(%s) not support to set netmask address.", netdev->name); rt_kprintf("\n"); } while (0);
        return -1;
    }

    if ((((netdev)->flags & 0x100U) ? (uint8_t)1 : (uint8_t)0))
    {
        do { rt_kprintf("[" "E" "/" "netdev" "] "); rt_kprintf("The network interface device(%s) DHCP capability is enable, not support set netmask address.", netdev->name); rt_kprintf("\n"); } while (0);
        return -1;
    }


    return netdev->ops->set_addr_info(netdev, 0, (ip_addr_t *)netmask, 0);
}
# 570 "../rt-thread/components/net/netdev/src/netdev.c"
int netdev_set_gw(struct netdev *netdev, const ip_addr_t *gw)
{
    if (!(netdev)) { rt_assert_handler("netdev", __FUNCTION__, 572); };
    if (!(gw)) { rt_assert_handler("gw", __FUNCTION__, 573); };

    if (!netdev->ops || !netdev->ops->set_addr_info)
    {
        do { rt_kprintf("[" "E" "/" "netdev" "] "); rt_kprintf("The network interface device(%s) not support to set gateway address.", netdev->name); rt_kprintf("\n"); } while (0);
        return -1;
    }

    if ((((netdev)->flags & 0x100U) ? (uint8_t)1 : (uint8_t)0))
    {
        do { rt_kprintf("[" "E" "/" "netdev" "] "); rt_kprintf("The network interface device(%s) DHCP capability is enable, not support set gateway address.", netdev->name); rt_kprintf("\n"); } while (0);
        return -1;
    }


    return netdev->ops->set_addr_info(netdev, 0, 0, (ip_addr_t *)gw);
}
# 601 "../rt-thread/components/net/netdev/src/netdev.c"
int netdev_set_dns_server(struct netdev *netdev, uint8_t dns_num, const ip_addr_t *dns_server)
{
    if (!(netdev)) { rt_assert_handler("netdev", __FUNCTION__, 603); };
    if (!(dns_server)) { rt_assert_handler("dns_server", __FUNCTION__, 604); };

    if (dns_num >= 2U)
    {
        do { rt_kprintf("[" "E" "/" "netdev" "] "); rt_kprintf("The number of DNS servers(%d) set exceeds the maximum number(%d).", dns_num + 1, 2U); rt_kprintf("\n"); } while (0);
        return -1;
    }

    if (!netdev->ops || !netdev->ops->set_dns_server)
    {
        do { rt_kprintf("[" "E" "/" "netdev" "] "); rt_kprintf("The network interface device(%s) not support to set DNS server address.", netdev->name); rt_kprintf("\n"); } while (0);
        return -1;
    }
...
void netdev_set_status_callback(struct netdev *netdev, netdev_callback_fn status_callback)
{
    if (!(netdev)) { rt_assert_handler("netdev", __FUNCTION__, 630); };
    if (!(status_callback)) { rt_assert_handler("status_callback", __FUNCTION__, 631); };
...
void netdev_set_addr_callback(struct netdev *netdev, netdev_callback_fn addr_callback)
{
    if (!(netdev)) { rt_assert_handler("netdev", __FUNCTION__, 644); };
    if (!(addr_callback)) { rt_assert_handler("addr_callback", __FUNCTION__, 645); };

    netdev->addr_callback = addr_callback;
}
# 658 "../rt-thread/components/net/netdev/src/netdev.c"
void netdev_low_level_set_ipaddr(struct netdev *netdev, const ip_addr_t *ip_addr)
{
    if (!(ip_addr)) { rt_assert_handler("ip_addr", __FUNCTION__, 660); };

    if (netdev && ((&(netdev->ip_addr))->addr == (ip_addr)->addr) == 0)
    {
        ((netdev->ip_addr).addr = (*ip_addr).addr);
# 675 "../rt-thread/components/net/netdev/src/netdev.c"
...
# 689 "../rt-thread/components/net/netdev/src/netdev.c"
void netdev_low_level_set_netmask(struct netdev *netdev, const ip_addr_t *netmask)
{
    if (!(netmask)) { rt_assert_handler("netmask", __FUNCTION__, 691); };

    if (netdev && ((&(netdev->netmask))->addr == (netmask)->addr) == 0)
    {
        ((netdev->netmask).addr = (*netmask).addr);
# 707 "../rt-thread/components/net/netdev/src/netdev.c"
        if (netdev->addr_callback)
        {
...
# 721 "../rt-thread/components/net/netdev/src/netdev.c"
void netdev_low_level_set_gw(struct netdev *netdev, const ip_addr_t *gw)
{
    if (!(gw)) { rt_assert_handler("gw", __FUNCTION__, 723); };

    if (netdev && ((&(netdev->gw))->addr == (gw)->addr) == 0)
    {
        ((netdev->gw).addr = (*gw).addr);
# 739 "../rt-thread/components/net/netdev/src/netdev.c"
        if (netdev->addr_callback)
        {
...
# 755 "../rt-thread/components/net/netdev/src/netdev.c"
void netdev_low_level_set_dns_server(struct netdev *netdev, uint8_t dns_num, const ip_addr_t *dns_server)
{
    unsigned int index;

    if (!(dns_server)) { rt_assert_handler("dns_server", __FUNCTION__, 759); };
    ...
    for (index = 0; index < 2U; index++)
    {
        if (((&(netdev->dns_servers[index]))->addr == (dns_server)->addr))
        {
            return;
        }
    }
    ...
    if (dns_num < 2U)
    {
        ((netdev->dns_servers[dns_num]).addr = (*dns_server).addr);
...
{
    struct netdev *new_netdev = 0;

    if (netdev->flags & 0x04U)
    {
        if (!(netdev_default->flags & 0x04U))
        {
    ...
    {
        new_netdev = netdev_get_first_by_flags(0x04U);
        if (new_netdev)
        {
...
# 818 "../rt-thread/components/net/netdev/src/netdev.c"
void netdev_low_level_set_status(struct netdev *netdev, rt_bool_t is_up)
{
    if (netdev && (((netdev)->flags & 0x01U) ? (uint8_t)1 : (uint8_t)0) != is_up)
    {
        if (is_up)
        {
            netdev->flags |= 0x01U;
...
# 851 "../rt-thread/components/net/netdev/src/netdev.c"
void netdev_low_level_set_link_status(struct netdev *netdev, rt_bool_t is_up)
{
    if (netdev && (((netdev)->flags & 0x04U) ? (uint8_t)1 : (uint8_t)0) != is_up)
    {
        if (is_up)
        {
            netdev->flags |= 0x04U;
# 866 "../rt-thread/components/net/netdev/src/netdev.c"
        }
        else
        {
            netdev->flags &= ~0x04U;
...
# 895 "../rt-thread/components/net/netdev/src/netdev.c"
void netdev_low_level_set_internet_status(struct netdev *netdev, rt_bool_t is_up)
{
    if (netdev && (((netdev)->flags & 0x80U) ? (uint8_t)1 : (uint8_t)0) != is_up)
    {
        if (is_up)
        {
            netdev->flags |= 0x80U;
...
# 923 "../rt-thread/components/net/netdev/src/netdev.c"
void netdev_low_level_set_dhcp_status(struct netdev *netdev, rt_bool_t is_enable)
{
    if (netdev && (((netdev)->flags & 0x100U) ? (uint8_t)1 : (uint8_t)0) != is_enable)
    {
        if (is_enable)
        {
            netdev->flags |= 0x100U;
...
#define NETDEV_IFCONFIG_IMEI_MAX_LEN 8
    ...
    for (node = &(cur_netdev_list->list); node; node = rt_slist_next(node))
    {
        ...
        rt_kprintf("\nFLAGS:");
        if (netdev->flags & 0x01U) rt_kprintf(" UP");
        else rt_kprintf(" DOWN");
        if (netdev->flags & 0x04U) rt_kprintf(" LINK_UP");
        else rt_kprintf(" LINK_DOWN");




        if (netdev->flags & 0x100U) rt_kprintf(" DHCP_ENABLE");
        else rt_kprintf(" DHCP_DISABLE");
        if (netdev->flags & 0x08U) rt_kprintf(" ETHARP");
        if (netdev->flags & 0x02U) rt_kprintf(" BROADCAST");
        if (netdev->flags & 0x20U) rt_kprintf(" IGMP");
        rt_kprintf("\n");
        rt_kprintf("ip address: %s\n", netdev_ip4addr_ntoa((const ip4_addr_t*)&(netdev->ip_addr)));
        rt_kprintf("gw address: %s\n", netdev_ip4addr_ntoa((const ip4_addr_t*)&(netdev->gw)));
        rt_kprintf("net mask  : %s\n", netdev_ip4addr_ntoa((const ip4_addr_t*)&(netdev->netmask)));
# 1043 "../rt-thread/components/net/netdev/src/netdev.c"
        for (index = 0; index < 2U; index++)
        {
            rt_kprintf("dns server #%d: %s\n", index, netdev_ip4addr_ntoa((const ip4_addr_t*)&(netdev->dns_servers[index])));
        }
...
# 1084 "../rt-thread/components/net/netdev/src/netdev.c"
static void netdev_set_if(char* netdev_name, char* ip_addr, char* gw_addr, char* nm_addr)
{
    ...
    if ((ip_addr != 0) && netdev_ip4addr_aton(ip_addr,(ip4_addr_t*)&addr))
    {
        netdev_set_ipaddr(netdev, &addr);
    }


    if ((gw_addr != 0) && netdev_ip4addr_aton(gw_addr,(ip4_addr_t*)&addr))
    {
        netdev_set_gw(netdev, &addr);
    }


    if ((nm_addr != 0) && netdev_ip4addr_aton(nm_addr,(ip4_addr_t*)&addr))
    {
...
# 1134 "../rt-thread/components/net/netdev/src/netdev.c"
    else if (argc == 5)
    {
        rt_kprintf("config : %s\n", argv[1]);
...
const char __fsym_ifconfig_name[] __attribute__((section(".rodata.name"))) = "ifconfig"; const char __fsym_ifconfig_desc[] __attribute__((section(".rodata.name"))) = "list the information of all network interfaces"; __attribute__((used)) const struct finsh_syscall __fsym_ifconfig __attribute__((section("FSymTab")))= { __fsym_ifconfig_name, __fsym_ifconfig_desc, (syscall_func)&netdev_ifconfig };;
...
#define NETDEV_PING_IS_COMMONICABLE(netdev) ((netdev) && (netdev)->ops && (netdev)->ops->ping && netdev_is_up(netdev) && netdev_is_link_up(netdev))
    ...
    if (!((netdev) && (netdev)->ops && (netdev)->ops->ping && (((netdev)->flags & 0x01U) ? (uint8_t)1 : (uint8_t)0) && (((netdev)->flags & 0x04U) ? (uint8_t)1 : (uint8_t)0)))
    {

        netdev = netdev_get_first_by_flags(0x04U);
        if (netdev == 0)
        {
            rt_kprintf("ping: not found available network interface device.\n");
            return -1;
        }
        else if (netdev->ops == 0 || netdev->ops->ping == 0)
        {
            rt_kprintf("ping: network interface device(%s) not support ping feature.\n", netdev->name);
            return -1;
        }
        else if (!(((netdev)->flags & 0x01U) ? (uint8_t)1 : (uint8_t)0) || !(((netdev)->flags & 0x04U) ? (uint8_t)1 : (uint8_t)0))
        {
    ...
    for (index = 0; index < times; index++)
    {
        ...
        ret = netdev->ops->ping(netdev, (const char *)target_name, size, (2 * 1000), &ping_resp);
        if (ret == -2)
        {
            rt_kprintf("ping: from %s icmp_seq=%d timeout\n",
                (((&(ping_resp.ip_addr)) == 
# 1221 "../rt-thread/components/net/netdev/src/netdev.c" 3 4
                ((void *)0) 
# 1221 "../rt-thread/components/net/netdev/src/netdev.c"
                || ((*(&(ping_resp.ip_addr))).addr == ((uint32_t)0x00000000UL)))) ? target_name : netdev_ip4addr_ntoa((const ip4_addr_t*)&(ping_resp.ip_addr)), index);
        }
        else if (ret == -1)
        {
            rt_kprintf("ping: unknown %s %s\n",
                (((&(ping_resp.ip_addr)) == 
# 1226 "../rt-thread/components/net/netdev/src/netdev.c" 3 4
                ((void *)0) 
# 1226 "../rt-thread/components/net/netdev/src/netdev.c"
                || ((*(&(ping_resp.ip_addr))).addr == ((uint32_t)0x00000000UL)))) ? "host" : "address",
                    (((&(ping_resp.ip_addr)) == 
# 1227 "../rt-thread/components/net/netdev/src/netdev.c" 3 4
                    ((void *)0) 
# 1227 "../rt-thread/components/net/netdev/src/netdev.c"
                    || ((*(&(ping_resp.ip_addr))).addr == ((uint32_t)0x00000000UL)))) ? target_name : netdev_ip4addr_ntoa((const ip4_addr_t*)&(ping_resp.ip_addr)));
        }
        else
        {
            if (ping_resp.ttl == 0)
            {
                rt_kprintf("%d bytes from %s icmp_seq=%d time=%d ms\n",
                            ping_resp.data_len, netdev_ip4addr_ntoa((const ip4_addr_t*)&(ping_resp.ip_addr)), index, ping_resp.ticks);
            }
            else
            {
                rt_kprintf("%d bytes from %s icmp_seq=%d ttl=%d time=%d ms\n",
                            ping_resp.data_len, netdev_ip4addr_ntoa((const ip4_addr_t*)&(ping_resp.ip_addr)), index, ping_resp.ttl, ping_resp.ticks);
            }
        }


        delay_tick = ((rt_tick_get() - start_tick) > (1 * 1000)) || (index == times) ? 0 : (1 * 1000);
...
const char __fsym_ping_name[] __attribute__((section(".rodata.name"))) = "ping"; const char __fsym_ping_desc[] __attribute__((section(".rodata.name"))) = "ping network host"; __attribute__((used)) const struct finsh_syscall __fsym_ping __attribute__((section("FSymTab")))= { __fsym_ping_name, __fsym_ping_desc, (syscall_func)&netdev_ping };;
...
static void netdev_set_dns(char *netdev_name, uint8_t dns_num, char *dns_server)
{
    ...
    netdev_ip4addr_aton(dns_server,(ip4_addr_t*)&dns_addr);
...
const char __fsym_dns_name[] __attribute__((section(".rodata.name"))) = "dns"; const char __fsym_dns_desc[] __attribute__((section(".rodata.name"))) = "list and set the information of dns"; __attribute__((used)) const struct finsh_syscall __fsym_dns __attribute__((section("FSymTab")))= { __fsym_dns_name, __fsym_dns_desc, (syscall_func)&netdev_dns };;
...
const char __fsym_netstat_name[] __attribute__((section(".rodata.name"))) = "netstat"; const char __fsym_netstat_desc[] __attribute__((section(".rodata.name"))) = "list the information of TCP / IP"; __attribute__((used)) const struct finsh_syscall __fsym_netstat __attribute__((section("FSymTab")))= { __fsym_netstat_name, __fsym_netstat_desc, (syscall_func)&netdev_netstat };;
