# 1 "../rt-thread/components/net/at/src/at_cli.c"
...
# 1 "<command-line>" 2
# 1 "../rt-thread/components/net/at/src/at_cli.c"
# 11 "../rt-thread/components/net/at/src/at_cli.c"
...
# 12 "../rt-thread/components/net/at/src/at_cli.c" 2
...
# 13 "../rt-thread/components/net/at/src/at_cli.c" 2
...
# 14 "../rt-thread/components/net/at/src/at_cli.c" 2
...
# 17 "../rt-thread/components/net/at/src/at_cli.c" 2
...
static struct rt_ringbuffer *console_rx_fifo = 0;
static rt_err_t (*odev_rx_ind)(rt_device_t dev, rt_size_t size) = 0;


static struct rt_semaphore client_rx_notice;
static struct rt_ringbuffer *client_rx_fifo = 0;
...
static char console_getchar(void)
{
    ...
    rt_sem_take(&console_rx_notice, -1);
...
void at_cli_init(void)
{
    ...
    rt_sem_init(&console_rx_notice, "cli_c", 0, 0x00);


    console_rx_fifo = rt_ringbuffer_create(256);

    if (!(console_rx_fifo)) { rt_assert_handler("console_rx_fifo", __FUNCTION__, 70); };
...
# 166 "../rt-thread/components/net/at/src/at_cli.c"
static char client_getchar(void)
{
    char ch;

    rt_sem_take(&client_rx_notice, -1);
...
#define DELECT_KEY 0x7F
    ...
    static rt_err_t (*client_odev_rx_ind)(rt_device_t dev, rt_size_t size) = 0;
    rt_base_t level;
    ...
    if (client)
    {
        ...
        rt_sem_init(&client_rx_notice, "cli_r", 0, 0x00);
        client_rx_fifo = rt_ringbuffer_create(256);

        at_client = rt_thread_create("at_cli", at_client_entry, 0, 512, 8, 8);
        if (client_rx_fifo && at_client)
        {
            ...
            while (0x1B != (ch = console_getchar()))
            {
                ...
                else if (ch == '\r' || ch == '\n')
                {
                    ...
                    {
                        rt_kprintf("\n");
                        at_obj_exec_cmd(client, 0, "%.*s", cur_line_len, cur_line);
                    }
                    cur_line_len = 0;
                }
                else
...
const char __fsym_at_name[] __attribute__((section(".rodata.name"))) = "at"; const char __fsym_at_desc[] __attribute__((section(".rodata.name"))) = "RT-Thread AT component cli: at <server|client [dev_name]>"; __attribute__((used)) const struct finsh_syscall __fsym_at __attribute__((section("FSymTab")))= { __fsym_at_name, __fsym_at_desc, (syscall_func)&at };;
