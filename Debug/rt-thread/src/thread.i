...
# 58 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD\\rt-thread\\include/rtdef.h" 2
...
# 22 "C:\\RT-ThreadStudio\\workspace\\AQMS_LCD\\rt-thread\\include/rthw.h" 2
...
# 39 "../rt-thread/src/thread.c" 2
# 1 "c:\\rt-threadstudio\\repo\\extract\\toolchain_support_packages\\wch\\risc-v-gcc-wch\\8.2.0\\lib\\gcc\\riscv-none-embed\\8.2.0\\include\\stddef.h" 1 3 4
# 40 "../rt-thread/src/thread.c" 2
...
static void _thread_timeout(void *parameter)
{
    ...
    if (!(thread != 0)) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 139); };
    if (!((thread->stat & 0x04) == 0x04)) { rt_assert_handler("(thread->stat & RT_THREAD_SUSPEND_MASK) == RT_THREAD_SUSPEND_MASK", __FUNCTION__, 140); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 141); };
                             ...
                             rt_uint32_t tick)
{
    ...
    if (!(priority < 32)) { rt_assert_handler("priority < RT_THREAD_PRIORITY_MAX", __FUNCTION__, 198); };
                        ...
                        rt_uint32_t tick)
{
    ...
    if (!(thread != 0)) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 328); };
    if (!(stack_start != 0)) { rt_assert_handler("stack_start != RT_NULL", __FUNCTION__, 329); };
...
rt_err_t rt_thread_startup(rt_thread_t thread)
{
    ...
    if (!(thread != 0)) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 379); };
    if (!((thread->stat & 0x07) == 0x00)) { rt_assert_handler("(thread->stat & RT_THREAD_STAT_MASK) == RT_THREAD_INIT", __FUNCTION__, 380); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 381); };
...
rt_err_t rt_thread_detach(rt_thread_t thread)
{
    ...
    if (!(thread != 0)) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 422); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 423); };
    if (!(rt_object_is_systemobject((rt_object_t)thread))) { rt_assert_handler("rt_object_is_systemobject((rt_object_t)thread)", __FUNCTION__, 424); };
...
rt_err_t rt_thread_delete(rt_thread_t thread)
{
    ...
    if (!(thread != 0)) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 535); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 536); };
    if (!(rt_object_is_systemobject((rt_object_t)thread) == 0)) { rt_assert_handler("rt_object_is_systemobject((rt_object_t)thread) == RT_FALSE", __FUNCTION__, 537); };
...
rt_err_t rt_thread_sleep(rt_tick_t tick)
{
    ...
    thread = rt_thread_self();
    if (!(thread != 0)) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 624); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 625); };


    do { if (1) { rt_bool_t interrupt_disabled; rt_base_t level; interrupt_disabled = rt_hw_interrupt_is_disabled(); level = rt_hw_interrupt_disable(); if (rt_critical_level() != 0) { rt_kprintf("Function[%s]: scheduler is not available\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 628); } } if (interrupt_disabled == 1) { rt_kprintf("Function[%s]: interrupt is disabled\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 628); } } do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_thread_self() == 0) { rt_kprintf("Function[%s] shall not be used before scheduler start\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 628); } } do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_interrupt_get_nest() != 0) { rt_kprintf("Function[%s] shall not be used in ISR\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 628); } } rt_hw_interrupt_enable(level); } while (0); rt_hw_interrupt_enable(level); } while (0); rt_hw_interrupt_enable(level); } } while (0);
...
rt_err_t rt_thread_delay_until(rt_tick_t *tick, rt_tick_t inc_tick)
{
    ...
    if (!(tick != 0)) { rt_assert_handler("tick != RT_NULL", __FUNCTION__, 694); };


    thread = rt_thread_self();
    if (!(thread != 0)) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 698); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 699); };
...
;
# 841 "../rt-thread/src/thread.c"
rt_err_t rt_thread_control(rt_thread_t thread, int cmd, void *arg)
{
    rt_base_t level;


    if (!(thread != 0)) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 846); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 847); };
...
static void rt_thread_set_suspend_state(struct rt_thread *thread, int suspend_flag)
{
    ...
    if (!(thread != 0)) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 945); };
    switch (suspend_flag)
    {
    ...
    case RT_UNINTERRUPTIBLE:
        ...
        break;
    default:
        if (!(0)) { rt_assert_handler("0", __FUNCTION__, 958); };
        break;
    }
    thread->stat = stat | (thread->stat & ~0x07);
}
# 980 "../rt-thread/src/thread.c"
rt_err_t rt_thread_suspend_with_flag(rt_thread_t thread, int suspend_flag)
{
    rt_base_t stat;
    rt_base_t level;
    ...
    if (!(thread != 0)) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 986); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 987); };
    if (!(thread == rt_thread_self())) { rt_assert_handler("thread == rt_thread_self()", __FUNCTION__, 988); };
    ...
    if (stat == 0x03)
    {
        ...
        if (!(thread == rt_thread_self())) { rt_assert_handler("thread == rt_thread_self()", __FUNCTION__, 1004); };
    }
# 1017 "../rt-thread/src/thread.c"
    rt_schedule_remove_thread(thread);
...
;
# 1045 "../rt-thread/src/thread.c"
rt_err_t rt_thread_resume(rt_thread_t thread)
{
    rt_base_t level;


    if (!(thread != 0)) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 1050); };
    if (!(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread)) { rt_assert_handler("rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread", __FUNCTION__, 1051); };
...
;
# 1161 "../rt-thread/src/thread.c"
rt_err_t rt_thread_get_name(rt_thread_t thread, char *name, rt_uint8_t name_size)
{
    return (thread == 0) ? -10 : rt_object_get_name(&thread->parent, name, name_size);
}
;
